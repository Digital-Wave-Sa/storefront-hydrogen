...
async function getShopifyMarketId(env: any, shopDomain: string, targetCountry: string): Promise<string | null> {
  const envMarketId = env?.SHOPIFY_MARKET_ID || env?.STOQ_MARKET_ID || env?.PUBLIC_STOQ_MARKET_ID;
  ...
  try {
    ...
    const res = await executeAdminQuery(marketsQuery, {}, adminToken, shopDomain);
    ...
    try {
      ...
      const v1Payload = {
        intent: {
          ...
          ...(numericLocationId && numericLocationId !== 'global' ? { shopify_location_id: Number(numericLocationId) || numericLocationId } : {}),
        ...
        customer: {
          ...
          ...(resolvedMarketId ? { shopify_market_id: String(resolvedMarketId) } : {}),
        },
      };
      ...
      if (!stoqRes.ok) {
        stoqRes = await fetch('https://app.stoqapp.com/api/v2/external/back_in_stock/signups', {
          method: 'POST',
          headers: stoqHeaders,
          body: JSON.stringify({
            channel: 'email',
