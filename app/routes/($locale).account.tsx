...
export async function loader({request, context}: LoaderFunctionArgs) {
  ...
  const {fetchWalletData} = await import('~/lib/account-wallet.server');
  ...
  try {
    ...
    const isVerifyPhoneRoute =
      pathname === `${localePrefix}/account/verify-phone` ||
      pathname === `${localePrefix}/account/verify-phone/`;
    const isLogoutRoute =
      pathname === `${localePrefix}/account/logout` ||
      pathname === `${localePrefix}/account/logout/`;
    ...
    console.warn(
      '[Account Loader] Storefront customer query failed, attempting Admin API fallback:',
      error,
    );
    try {
      const savedPhone = await session.get('loginOtpPhone');
      const savedEmail = await session.get('loginCustomerEmail');
      let savedCustomerId = await session.get('loginCustomerId');
      
      const customerAccessToken = await session.get('customerAccessToken');
      const tokenStr = typeof customerAccessToken === 'string' ? customerAccessToken : customerAccessToken?.accessToken;
      if (!savedCustomerId && tokenStr && tokenStr.startsWith('session-')) {
        savedCustomerId = tokenStr.replace('session-', '');
      }

      let adminToken: string | null = null;
      let adminDomain: string | null = null;
      let adminCust: any = null;
      let adminAddresses: any[] = [];

      try {
        const {getAdminToken} = await import('~/lib/shopify-admin.server');
        adminDomain = context.env.PUBLIC_STORE_DOMAIN;
        adminToken = await getAdminToken(context.env);
      } catch (e) {
        console.error('[Account Loader] Failed to get Admin API token:', e);
      }
        ...
        if (savedCustomerId) {
          try {
            const res = await fetch(
              `https://${adminDomain}/admin/api/2024-01/customers/${savedCustomerId}.json`,
              {
        ...
        if (adminCust?.id) {
          try {
            const addrRes = await fetch(
              `https://${adminDomain}/admin/api/2024-01/customers/${adminCust.id}/addresses.json`,
              {
                headers: {'X-Shopify-Access-Token': adminToken},
              },
            );
      ...
      const tags = adminCust?.tags
        ? typeof adminCust.tags === 'string'
          ? adminCust.tags.split(',').map((t: string) => t.trim())
          : adminCust.tags
        : [];
      const isAdmin = tags.some((tag: string) => {
      ...
      if (adminCust?.id && adminToken && adminDomain) {
        try {
          ...
          console.error(
            '[Account Loader] Failed to fetch customer orders via Admin API:',
            e,
          );
        }
      }
      ...
      const walletPromise = fetchWalletData({
...
function AccountLayout({
  ...
  const isAccountHome =
    location.pathname === `${localePrefix}/account` ||
    location.pathname === `${localePrefix}/account/`;
