...
export function NoSearchResults({ searchTerm }: { searchTerm: string }) {
  ...
  return (
    <div className={`flex flex-col items-center justify-center text-center py-20 px-4 sm:px-12 bg-white rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden ${isEn ? 'font-en' : "font-['GE_Dinar_One']"}`} dir={isEn ? 'ltr' : 'rtl'}>
      {/* Decorative Brand Background */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-[#9FB7AE]/10 rounded-bl-full -z-0"></div>
      <div className="absolute bottom-0 left-0 w-48 h-48 bg-[#9FB7AE]/10 rounded-tr-full -z-0"></div>
      ...
      <div className="relative z-10 max-w-[500px] mb-10">
        <p className="text-[#7D7D7D] text-[16px] leading-[1.6]">
          {searchTerm
...
export function PredictiveSearchResults({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div className="predictive-search-results">
      ...
      {searchTerm.current && (
        <Link onClick={goToSearchResult} to={`${locale === 'en' ? '/en' : ''}/search?q=${searchTerm.current}`}>
          <p>
