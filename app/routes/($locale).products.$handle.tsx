...
export const meta: MetaFunction<typeof loader> = ({data}) => {
  ...
  const title =
    product.seo?.title ||
    `${product.title}${priceString ? ` - ${priceString}` : ''} | Saadeddin`;
  const description =
    product.seo?.description ||
    product.description?.substring(0, 155) ||
    `Buy ${product.title} at Saadeddin. Freshly prepared daily.`;
...
export async function loader(args: LoaderFunctionArgs) {
  ...
  if (!product?.id) {
    ...
    if (fallbackProduct?.id) {
      ...
      const {product: currentLocaleProduct} = await storefront.query(
        `#graphql
    ...
    await Promise.all([
      ...
      (async () => {
        ...
        try {
          ...
          const reviewsResult = (await adminApiQuery(
            shopDomain,
            adminToken,
            reviewsQuery,
          )) as any;
          ...
          reviews = allReviews.filter((r: any) => {
            ...
            const isApproved =
              ...
              !statusStr;
            return (
              (rHandle === targetHandle ||
                rHandle === (decodedHandle || '').toLowerCase().trim()) &&
              isApproved
            );
          });
        } catch (err) {
      ...
      (async () => {
        ...
        if ((product.related_products as any)?.references?.nodes?.length > 0) {
          recommended.products.nodes = (
            product.related_products as any
          ).references.nodes
            .map((node: any) => {
        ...
        if (
          recommended.products.nodes.length === 0 &&
          product.collections?.nodes?.[0]?.id
        ) {
          try {
            const collectionResult = await storefront.query(
            ...
            if (
              (collectionResult as any).collection?.products?.nodes?.length > 0
            ) {
              recommended.products.nodes = (
...
export default function Product() {
  const {
    ...
    variants,
    visibility,
    reviews: loaderReviews,
    dynamicRating: loaderRating,
  ...
  const rootReviews = (rootData?.reviews?.nodes || [])
    .map((node: any) => {
      const f: any = {};
      node.fields.forEach((field: any) => (f[field.key] = field.value));
      return f;
  ...
  const reviews =
    Array.isArray(loaderReviews) && loaderReviews.length > 0
      ? loaderReviews
      : rootReviews;
  ...
  const {selectedLocationId, selectedLocationName} = useOutletContext<{
  ...
  const variantComponents = ((selectedVariant as any)?.components?.nodes || [])
    .map((c: any) => {
  ...
  const bundleComponents =
    variantComponents.length > 0 ? variantComponents : rawMetaComponents;
  const isBundle =
    product.productType?.toLowerCase() === 'bundle' ||
    product.tags?.some((t: string) => t.toLowerCase() === 'bundle') ||
    ...
    const componentsTotal = bundleComponents.reduce(
      (sum: number, component: any) => {
        const price = parseFloat(
          component.variants?.nodes[0]?.price?.amount || '0',
        );
        return sum + price;
      },
      0,
  ...
  const isCorporateProduct =
    product.tags?.some((t: string) => {
      ...
      return (
        lowerTag.includes('corporate') ||
        lowerTag.includes('b2b') ||
        lowerTag.includes('package')
      );
    }) ||
    ['classic-collection', 'premium-collection', 'custom-collection'].includes(
      product.handle,
    );
  ...
  const isGiftable =
    product.tags?.some((t: string) => {
      ...
      return (
        ...
        lowerTag.includes('mother') ||
        lowerTag.includes('father') ||
        lowerTag.includes('friend') ||
        lowerTag.includes('grad')
      );
    }) ?? false;
      ...
      const mainLine = {
        ...
        attributes: [
          ...
          ...(isBundle && bundleComponents.length > 0
            ? [
                ...
                },
                {
                  key: '_bundle_skus',
                  value: bundleComponents
                    .map((c: any) => c.variants?.nodes?.[0]?.sku)
                    .filter(Boolean)
                    .join(', '),
                },
              ]
          ...
          ...(isGiftMode
            ...
            : note
              ? [{key: 'Order Note', value: note}]
              : []),
        ],
      };

      const addonLines = selectedAddons.map((addonId) => {
        const addonNode = addonNodes.find(
          (n: any) => n.variants?.nodes?.[0]?.id === addonId,
        );
  ...
  const estimatedDeliveryDate = useMemo(() => {
    ...
    if (leadTimeHours === null) {
      const collectionLeadTimes =
        (product as any).collections?.nodes?.map((n: any) =>
          n.leadTime?.value ? parseInt(n.leadTime.value) : 0,
        ) || [];
      ...
      <div className="flex flex-col gap-[32px] w-full mt-[16px]">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center md:items-start w-full gap-8 md:gap-4">
          ...
          <div className="flex flex-col items-center gap-[4px]">
            <h3
              className="text-[48px] font-bold text-[#171717] leading-none m-0 p-0"
              style={{fontFamily: "'Inter', 'Bahij Janna', sans-serif"}}
              dir="ltr"
            >
            ...
            <span
              className="text-[12px] text-[#7D7D7D] font-normal mt-[8px]"
              style={{
          ...
          <div className="flex flex-col gap-[8px] w-full md:w-[320px]">
            {[5, 4, 3, 2, 1].map((star) => {
              const count = reviews.filter(
                (r: any) => Math.round(parseFloat(r.rating)) === star,
              ).length;
              const percentage =
              ...
              return (
                <div key={star} className="flex items-center gap-[12px]">
                  ...
                  <div className="flex items-center gap-[4px] w-[28px] justify-end">
                    <span
                      className="text-[12px] font-medium text-[#7D7D7D]"
                      style={{
                        fontFamily:
                  ...
                  {/* Count */}
                  <div
                    className="w-[20px] text-[12px] font-medium text-[#7D7D7D] text-start"
                    style={{
                      fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
                    }}
                  >
          ...
          {hasPurchased ? (
            ...
            >
              {isEn
                ...
                : 'فقط المشترين يمكنهم إضافة مراجعة'}
            </span>
          )}
        </div>
        ...
        {showReviewForm && (
          <div className="bg-white p-8 rounded-[16px] border border-[#E5E5E5] shadow-sm animate-fade-in relative mb-[16px] w-full text-start">
            <div className="flex justify-between items-center mb-6">
              <h4
                className="text-[18px] font-bold text-[#234745]"
                style={{
                  fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
                }}
              >
              ...
              >
                <div className="w-full flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-[12px]">
                    <div
                      className="w-[40px] h-[40px] bg-[#BBCFCD] rounded-full flex items-center justify-center text-[#255441] text-[18px] font-bold"
                      style={{
                        fontFamily:
                          "'EnglishDigits', 'GE Dinar One', sans-serif",
                        paddingTop: '2px',
                    ...
                    <div className="flex flex-col items-start">
                      <h5
                        className="text-[16px] font-bold text-[#255441] leading-tight"
                        style={{
                          fontFamily:
                            "'EnglishDigits', 'GE Dinar One', sans-serif",
                        }}
                      ...
                      <span
                        className="text-[14px] text-[#BBCFCD] font-normal"
                        style={{
                          fontFamily:
                            "'EnglishDigits', 'GE Dinar One', sans-serif",
                        }}
                ...
                <div className="w-full flex justify-start pr-[52px]">
                  <div className="flex items-center gap-[6px]">
                    ...
                    >
                      <path
                        d="M14.6666 7.38674V8.00007C14.6658 9.43769 14.2003 10.8365 13.3395 11.9879C12.4788 13.1394 11.2688 13.9818 9.88907 14.3893C8.50931 14.7968 7.03402 14.7471 5.68417 14.2491C4.33433 13.751 3.18187 12.8315 2.40049 11.6288C1.61912 10.4261 1.25143 8.97298 1.35245 7.51004C1.45347 6.04711 2.01827 4.65089 2.96162 3.52355C3.90497 2.3962 5.17646 1.59591 6.58882 1.23846C8.00119 0.881006 9.48003 0.98592 10.8133 1.5334"
                        stroke="#255441"
                        strokeWidth="1.2"
                        strokeLinecap="round"
                      ...
                      />
                      <path
                        d="M14.6666 2.66675L8.00001 9.34008L5.99999 7.34008"
                        stroke="#255441"
                        strokeWidth="1.2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                    ...
                    <span
                      className="text-[#255441] text-[12px] font-bold"
                      style={{
                        fontFamily:
                          "'EnglishDigits', 'GE Dinar One', sans-serif",
                      }}
  ...
  return (
    <div
      key={product.id}
      dir={isEn ? 'ltr' : 'rtl'}
      className={`w-full bg-[#fafafa] ${isEn ? 'font-en' : 'font-ar'}`}
    >
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            offers: {
              ...
              availability: !effectiveOutOfStock
      ...
      <div className="w-full">
        ...
        >
          ...
          />
          <div
            className="relative z-10 w-full max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12 flex items-center justify-between"
            dir={isEn ? 'ltr' : 'rtl'}
          >
            <button
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 pt-8 md:pt-12 pb-[64px] lg:pb-[64px] grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="lg:col-span-5 flex flex-col pt-2 order-2">
            ...
            {(() => {
              ...
              if (primaryCollection) {
                if (isEn) {
                  ...
                  const hasArabicInTitle = /[\u0600-\u06FF]/.test(
                    primaryCollection.title || '',
                  );
                  colTitle =
                    arMetafield && String(arMetafield).trim()
              ...
              const collectionUrl = primaryCollection?.handle
                ? isEn
                  ? `/en/collections/${primaryCollection.handle}`
            ...
            <div className="flex flex-wrap items-center gap-[12px] mb-[24px] w-full justify-start">
              ...
              <div className="flex items-center gap-[16px]">
                ...
                <div className="flex items-center gap-[9px]">
                  <span
                    className="text-[#171717] text-[12px] font-bold"
                    style={{
                      fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
                      lineHeight: '100%',
                    }}
                  ...
                  <span
                    className="text-[#7D7D7D] text-[12px] font-normal"
                    style={{
                      fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
                      lineHeight: '100%',
                    }}
                  >
                    {isEn
                      ? `(${dynamicCount || 0} Reviews)`
                      : `(${new Intl.NumberFormat('en-US').format(dynamicCount || 0)} مراجعة)`}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-[6px]">
                <span
                  className={`w-[6px] h-[6px] rounded-full ${effectiveOutOfStock ? 'bg-[#E64950]' : 'bg-[#255441]'}`}
                ></span>
                <span
                  className={`text-[12px] font-bold ${effectiveOutOfStock ? 'text-[#E64950]' : 'text-[#255441]'}`}
                  style={{
                    fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
                    lineHeight: '100%',
                  }}
            ...
            >
              {product.seo?.description ||
                (product.description && product.description.length > 150
                  ...
                  : product.description) ||
                (isEn
                  ? 'Saadeddin offers the finest chocolate selection, prepared with passion and modern techniques.'
                  : 'تشكيلة فاخرة من أجود أنواع الشوكولاتة المختارة، محضّرة بأحدث التقنيات مع محافظة تامة على الطعم الأصيل والجودة العالية.')}
            </p>

            {/* Tags */}
            {((product as any).vegan?.value === 'true' ||
              (product as any).vegan?.value === '1' ||
              (product as any).lactose_free?.value === 'true' ||
              (product as any).lactose_free?.value === '1' ||
              (product as any).gluten_free?.value === 'true' ||
              (product as any).gluten_free?.value === '1') && (
              <div
                className="flex flex-wrap items-center gap-[12px] mb-[24px] w-full"
              ...
              >
                {((product as any).vegan?.value === 'true' ||
                  ...
                  <div className="h-[40px] px-[16px] bg-[#FEF8EB] rounded-[25px] border border-[#BBCFCD]/50 flex items-center justify-center whitespace-nowrap">
                    <span
                      className="text-[#255441] text-[14px] font-normal"
                      style={{
                        fontFamily:
                          "'EnglishDigits', 'GE Dinar One', sans-serif",
                      }}
                    >
                      {isEn ? 'Vegan 100%' : 'نباتي 100%'}
                    </span>
                  </div>
                )}
                {((product as any).lactose_free?.value === 'true' ||
                  ...
                  <div className="h-[40px] px-[16px] bg-[#FEF8EB] rounded-[25px] border border-[#BBCFCD]/50 flex items-center justify-center whitespace-nowrap">
                    ...
                    >
                      {isEn ? 'Lactose Free' : 'خالٍ من اللاكتوز'}
                    </span>
                  </div>
                )}
                {((product as any).gluten_free?.value === 'true' ||
                  ...
                  <div className="h-[40px] px-[16px] bg-[#FEF8EB] rounded-[25px] border border-[#BBCFCD]/50 flex items-center justify-center whitespace-nowrap">
                    ...
                    >
                      {isEn ? 'Gluten Free' : 'خالٍ من الغلوتين'}
                    </span>
                  </div>
                )}
              </div>
          ...
          <Suspense
            fallback={
              <div>
                {isEn ? 'Loading options...' : 'جاري تحميل الخيارات...'}
              </div>
            }
          >
            <Await resolve={variants}>
              {(data) => (
                <div className="flex flex-col gap-8">
                  ...
                  {isBundle && bundleComponents.length > 0 && (
                    <div
                      className="flex flex-col gap-6 mt-4 bg-[#FEF8EB] p-6 rounded-[24px] border border-[#BBCFCD]/50 shadow-sm relative overflow-hidden w-full"
                    ...
                    >
                      ...
                      <div className="flex flex-row items-center justify-between relative z-10">
                        <div className="flex flex-row items-center gap-3">
                          ...
                          <h5
                            className="font-bold text-[#234745] text-[20px]"
                            style={{
                              fontFamily:
                                "'EnglishDigits', 'Bahij Janna', sans-serif",
                            }}
                        ...
                        {bundleSavings && bundleSavings > 0 && (
                          <div className="flex flex-col items-end">
                            <span
                              className="text-[#906B51] text-[12px] font-bold mb-1"
                              style={{
                                fontFamily:
                                  "'EnglishDigits', 'GE Dinar One', sans-serif",
                              }}
                            ...
                            <span
                              className="bg-[#234745] text-white text-[14px] font-bold px-3 py-1 rounded-[8px] shadow-sm"
                              style={{
                                fontFamily:
                                  "'EnglishDigits', 'GE Dinar One', sans-serif",
                              }}
                              ...
                              >
                                ...
                                <div className="flex flex-row items-center gap-4 bg-white border border-[#BBCFCD]/30 rounded-[16px] p-4 shadow-sm transition-all hover:border-[#234745] hover:shadow-md">
                                  ...
                                  <div className="flex flex-col flex-grow text-start">
                                    <span
                                      className="text-[16px] font-bold text-[#1a1a1a] line-clamp-2 leading-tight"
                                      style={{
                                        fontFamily:
                                          "'EnglishDigits', 'GE Dinar One', sans-serif",
                                      }}
                                    ...
                                    {compPrice && (
                                      <div
                                        className="flex flex-row items-center gap-1 text-[#906B51] text-[14px] font-bold mt-2 justify-start"
                                        style={{
                                          fontFamily:
                                            "'EnglishDigits', 'GE Dinar One', sans-serif",
                                        }}
                  ...
                  {addonNodes.length > 0 && (
                    <div className="flex flex-col gap-[16px] w-full max-w-[519px]">
                      <h5
                        className="font-bold text-[#255441] text-[18px] !m-0 text-start"
                        style={{
                          fontFamily:
                            "'EnglishDigits', 'GE Dinar One', sans-serif",
                        }}
                      ...
                      <div className="flex flex-col gap-[8px]">
                        {addonNodes.map((addon: any) => {
                          ...
                          return (
                            <div
                              key={addon.id}
                              className={`w-full h-[64px] px-[16px] rounded-[12px] border transition-all flex items-center justify-between cursor-pointer ${
                                isSelected
                                  ? 'border-[#234745] bg-[#f0f4f2]'
                                  : 'border-[#D2D2D2] bg-white hover:border-gray-300'
                              } ${outOfStock ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
                              onClick={() =>
                                !outOfStock && handleAddonToggle(variant.id)
                              }
                            >
                              <div className="flex items-center gap-[12px] flex-row">
                                ...
                                <div
                                  className={`w-[20px] h-[20px] rounded-[4px] border flex items-center justify-center transition-colors ${
                                    isSelected
                                      ? 'bg-[#255441] border-[#255441]'
                                      : 'border-[#9FB7AE] bg-white'
                                  }`}
                                >
                                  {isSelected && (
                                    <svg
                                ...
                                >
                                  {isEn
                                    ? addon.title
                                    : addon.title === ' متوفر الان'
                                      ? 'متوفر الان'
                                      : addon.title}
                                </p>
                              </div>
                              <div
                                className="flex items-center gap-1 text-[16px] text-[#255441] font-bold"
                                style={{
                                  fontFamily:
                                    "'EnglishDigits', 'GE Dinar One', sans-serif",
                                }}
                    ...
                    <div className="w-full mt-[8px] max-w-[519px]">
                      <div className="bg-white border border-[#E5E5E5] rounded-[16px] p-[16px] flex items-center justify-between">
                        ...
                        <div className="flex flex-col justify-center items-start">
                          ...
                          >
                            {isEn
                              ? 'Add a message and special packaging for the recipient'
                              : 'أضف رسالة وتغليف مميز للمستلم'}
                          </span>
                        </div>
                        ...
                        >
                          <div
                            className={`bg-[#FEF8EB] w-[24px] h-[24px] rounded-full shadow-sm transform transition-transform duration-300 ${isGiftMode ? (isEn ? 'translate-x-[24px]' : 'translate-x-[-24px]') : 'translate-x-0'}`}
                          ></div>
                        </div>
                      </div>
                      ...
                      {isGiftMode && (
                        <div className="mt-[16px] flex flex-col gap-4 animate-fade-in p-[16px] bg-[#fdfdfd] border border-gray-100 rounded-[12px]">
                          <div className="flex flex-col gap-2">
                            <label className="text-[13px] font-bold text-[#1a1a1a] text-start">
                              {isEn ? 'Recipient Name' : 'اسم المستلم'}
                            </label>
                            <input
                              type="text"
                              value={recipientName}
                              ...
                              maxLength={30}
                              className="w-full p-3 text-[14px] border border-[#BBCFCD]/50 rounded-[8px] focus:ring-[#234745] focus:border-[#234745] bg-white font-medium text-start"
                              placeholder={isEn ? 'e.g. Sarah' : 'مثال: سارة'}
                            />
                          </div>
                          <div className="flex flex-col gap-2">
                            <label className="text-[13px] font-bold text-[#1a1a1a] text-start">
                              {isEn ? 'Gift Message' : 'رسالة إهداء'}
                            </label>
                            <textarea
                              value={note}
                              ...
                              rows={3}
                              maxLength={150}
                              className="w-full p-3 text-[14px] border border-[#BBCFCD]/50 rounded-[8px] focus:ring-[#234745] focus:border-[#234745] resize-none bg-white font-medium text-start"
                              placeholder={
                                isEn
                          ...
                          <label className="flex items-center gap-3 cursor-pointer group flex-row">
                            <input
                              ...
                              className="w-[18px] h-[18px] rounded-[4px] text-[#234745] focus:ring-[#234745] border-[#BBCFCD]/50"
                            />
                            <span className="text-[13px] font-medium text-[#7D7D7D] group-hover:text-[#234745] transition-colors">
                              {isEn
                                ? 'Hide my name (Anonymous Gift)'
                                : 'إخفاء اسمي (هدية سرية)'}
                            </span>
                          </label>
                        </div>
                      )}
                    ...
                    <div className="w-full mt-[8px] max-w-[519px]">
                      <div className="bg-[#FEF8EB] border border-[#E5E5E5] rounded-[16px] p-[16px] flex flex-col items-start gap-4 shadow-sm relative overflow-hidden">
                        <div className="absolute top-[10px] right-[-10px] opacity-10 pointer-events-none transform -rotate-12">
                          ...
                          >
                            <path
                              d="M12 4.5C12 4.5 9.5 2 6 2C2.5 2 2 6 2 6C2 6 2.5 8 6 8C9.5 8 12 4.5 12 4.5Z"
                              fill="#234745"
                            />
                            <path
                              d="M12 4.5C12 4.5 14.5 2 18 2C21.5 2 22 6 22 6C22 6 21.5 8 18 8C14.5 8 12 4.5 12 4.5Z"
                              fill="#234745"
                            />
                        ...
                        <input
                          ...
                          placeholder={
                            writeLocation === 'cake'
                              ? isEn
                                ? 'e.g. Happy Birthday (written on cake)'
                      ...
                      <div className="bg-[#FEF8EB] border border-[#E6E2D8] rounded-[16px] p-[16px] flex flex-col items-start gap-4 shadow-sm relative overflow-hidden">
                        <div className="flex flex-col justify-center items-start z-10 w-full text-start">
                          ...
                          >
                            ...
                            <span>
                              {isEn
                                ? 'Corporate Customization & Branding'
                                : 'تخصيص وشعار الشركة'}
                            </span>
                          </span>
                          <span
                            ...
                            {companyLogoName ? (
                              <div className="flex items-center gap-2 text-emerald-700 font-bold text-[14px]">
                                ...
                                <span className="text-[12px] text-gray-500 font-normal">
                                  ({isEn ? 'Click to change' : 'اضغط للتغيير'})
                                </span>
                              </div>
                            ) : (
                              <div className="flex flex-col items-center gap-1">
                                ...
                                <span className="text-[#8B9895] text-[12px]">
                                  {isEn
                                    ? 'PNG, JPG, SVG, or PDF (Max 10MB)'
                                    : 'صورة PNG، JPG، SVG أو ملف PDF'}
                                </span>
                              </div>
                            )}
                          </label>
          ...
          <div className="lg:sticky lg:top-24 flex flex-col gap-4 w-full lg:w-[301px]">
            ...
            {effectiveOutOfStock ? (
              isVisibilityBlocked ? (
                ...
                (() => {
                  const seasonMsg =
                    (product as any).seasonal_message?.value ||
                    (product as any).next_season_date?.value ||
                    null;
                  return (
                    <div className="hidden lg:flex bg-white rounded-[24px] border border-[#BBCFCD]/60 p-6 flex-col items-start justify-center text-start gap-3 w-full shadow-sm">
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span
                          className="text-[#171717] font-medium text-[16px] sm:text-[16px]"
                          style={{
                            fontFamily:
                              "'EnglishDigits', 'GE Dinar One', sans-serif",
                          }}
                      ...
                      {seasonMsg && (
                        <div
                          className="text-[#9FB7AE] text-[16px] font-medium"
                          style={{
                            fontFamily:
                              "'EnglishDigits', 'Bahij Janna', sans-serif",
                          }}
                      ...
                      >
                        ...
                        >
                          {isEn
                            ? 'Notify Me for Next Season'
                            : 'أبلغني في الموسم القادم'}
                        </span>
                      </button>
                ...
                (() => {
                  const restockDate =
                    (product as any).restock_date?.value ||
                    (product as any).expected_restock_date?.value ||
                    null;
                  let formattedRestock: string | null = null;
                  if (restockDate) {
                    try {
                      const d = new Date(restockDate);
                      formattedRestock = d.toLocaleDateString(
                        isEn ? 'en-SA' : 'ar-SA-u-nu-latn',
                        {year: 'numeric', month: 'long', day: 'numeric'},
                      );
                    } catch (_) {
                      formattedRestock = restockDate;
                    }
                    if (formattedRestock) {
                  ...
                  return (
                    <div className="hidden lg:flex bg-white rounded-[20px] border border-[#BBCFCD]/50 p-[16px] flex-col items-start justify-center text-start gap-3 w-full">
                      ...
                      {formattedRestock && (
                        <div
                          className="text-[#906B51] text-[16px] font-bold"
                          style={{
                            fontFamily:
                              "'EnglishDigits', 'Bahij Janna', sans-serif",
                          }}
                        >
                          {isEn
                            ? `Expected: ${formattedRestock}`
                            : `متوقع توفره: ${formattedRestock}`}
                        </div>
                      )}
              ...
              <div className="hidden lg:flex bg-white rounded-[20px] p-4 border border-[#BBCFCD]/50 flex-col gap-6 w-full">
                ...
                <div className="bg-[#FEF8EB] rounded-[16px] py-[12px] px-[16px] border border-[#BBCFCD]/50 flex flex-col items-center justify-center gap-2">
                  <div className="flex items-center justify-center gap-[16px] w-full h-[32px]">
                    ...
                    />
                    <img
                      src="/images/icons/mada.png"
                      className="h-[24px] object-contain scale-[1.7] origin-center"
                      alt="Mada"
                    />
                  </div>
                  <span
                    className="text-[14px] font-bold text-[#234745] text-center"
                    style={{
                      fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
                    }}
                  >
                ...
                <div className="flex items-center justify-between h-[40px] w-full">
                  <span
                    className="font-medium text-[#171717] text-[16px]"
                    style={{
                      fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
                    }}
                  >
                  ...
                  <div className="flex items-center gap-[8px]">
                    ...
                    <div
                      className="w-[40px] h-[40px] flex items-center justify-center bg-white rounded-[8px] border border-[#BBCFCD]/50 font-medium text-[16px] text-[#255441]"
                      style={{
                        fontFamily:
                          "'EnglishDigits', 'GE Dinar One', sans-serif",
                      }}
                            ...
                            ? (() => {
                                ...
                                const addonLines = selectedAddons.map(
                                  (addonId) => {
                                    const addonNode = addonNodes.find(
                                      (n: any) =>
                                        n.variants.nodes[0].id === addonId,
                                    );
                                    const variant =
                                      addonNode?.variants.nodes[0];
                                    return {
                                      merchandiseId: addonId,
                                      quantity: 1,
            ...
            <div className="lg:hidden bg-white fixed bottom-0 left-0 right-0 z-40 rounded-t-[32px] px-[16px] py-[24px] shadow-[0_-10px_30px_rgba(0,0,0,0.08)] border-t border-[#BBCFCD]/30 flex flex-col gap-4">
              <div className="flex flex-col gap-4">
                ...
                {!effectiveOutOfStock && (
                  <div className="flex items-center justify-between h-[40px] w-full">
                    <span
                      className="font-bold text-[#171717] text-[16px]"
                      style={{
                        fontFamily:
                          "'EnglishDigits', 'GE Dinar One', sans-serif",
                      }}
                    ...
                    <div className="flex items-center gap-[8px]">
                      ...
                      <div
                        className="w-[40px] h-[40px] flex items-center justify-center bg-white rounded-[8px] border border-[#BBCFCD]/50 font-bold text-[16px] text-[#255441]"
                        style={{
                          fontFamily:
                            "'EnglishDigits', 'GE Dinar One', sans-serif",
                        }}
                      ...
                      return (
                        <div className="w-full flex flex-col items-center justify-center text-center gap-3 py-1">
                          ...
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <span
                              className="text-[#171717] font-bold text-[18px]"
                              style={{
                                fontFamily:
                                  "'EnglishDigits', 'GE Dinar One', sans-serif",
                              }}
                          ...
                          {seasonMsg && (
                            <div
                              className="text-[#967459] text-[15px] font-bold"
                              style={{
                                fontFamily:
                                  "'EnglishDigits', 'GE Dinar One', sans-serif",
                              }}
                      ...
                      return (
                        <div className="w-full flex flex-col items-center justify-center text-center gap-3 py-1">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <span
                              className="text-[#171717] font-medium text-[16px]"
                              style={{
                                fontFamily:
                                  "'EnglishDigits', 'GE Dinar One', sans-serif",
                              }}
              ...
              {!effectiveOutOfStock && (
                <div className="bg-[#FEF8EB] rounded-[16px] py-[10px] px-[16px] border border-[#BBCFCD]/50 flex flex-col items-center justify-center w-full">
                  <div className="flex items-center justify-center gap-[16px] w-full h-[24px]">
                    ...
                    <img
                      src="/images/icons/mada.png"
                      className="h-[20px] object-contain scale-[1.5] origin-center"
                      alt="Mada"
                    />
                  </div>
                </div>
            ...
            {(() => {
              const locations =
                rootData?.locations?.locations?.nodes ||
                rootData?.locations?.nodes ||
                [];
              const currentBranch = locations.find(
                (loc: any) =>
          ...
          <div className="hidden md:flex flex-col w-full gap-[24px] items-start">
            ...
            {(() => {
              const tabs = [
                {
                  id: 'details',
                  label: isEn ? 'Product Description' : 'وصف المنتج',
                },
                {
                  id: 'reviews',
                  label: isEn
                    ? `Reviews (${reviews?.length || 0})`
                    : `المراجعات (${new Intl.NumberFormat('en-US').format(reviews?.length || 0)})`,
                },
                ...((product as any).nutrition?.value
                  ? [
                      {
                        id: 'nutrition',
                        label: isEn ? 'Nutrition Facts' : 'حقائق غذائية',
                      },
                    ]
                  : []),
              ];
                ...
                <div className="w-full flex flex-col gap-[16px] items-start relative">
                  ...
                  <div className="w-full relative h-[2px]">
                    ...
                    <div
                      ...
                      style={{
                        [isEn ? 'left' : 'right']:
                          `${tabs.findIndex((t) => t.id === activeTab) * 176}px`,
                      }}
                    ></div>
                  </div>
                </div>
            ...
            <div className="w-full animate-fade-in text-start">
              {activeTab === 'details' ? (
                <div className="w-full">
                  ...
                  {(product as any).allergens?.value && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div
                        className={`p-8 bg-orange-50/50 rounded-[32px] border border-orange-100/50 ${isEn ? 'text-left' : 'text-right'}`}
                      >
                ...
                <div className="w-full">
                  {(product as any).nutrition?.value && (
                    <div
                      className={`p-8 bg-[#295b45]/5 rounded-[32px] border border-[#295b45]/10 max-w-2xl ${isEn ? 'text-left' : 'text-right'}`}
                    >
                    ...
                    {mobileTabs.map((t) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() => setActiveTab(t.id)}
                          className={`relative pb-3 text-[14px] xs:text-[15px] sm:text-[17px] font-bold transition-all flex flex-col items-center whitespace-nowrap shrink-0 ${
                            isActive
                              ? 'text-[#234745]'
                              : 'text-[#8B9895] hover:text-[#234745]'
                          }`}
                          style={{
                            fontFamily:
                              "'EnglishDigits', 'GE Dinar One', sans-serif",
        ...
        <div className="max-w-[1400px] mx-auto px-4 md:px-8">
          <div className="flex items-center justify-between mb-[32px] w-full">
            ...
            >
              ...
              >
                <path
                  d="M4.5 8.5L0.5 4.5L4.5 0.5M0.5 4.5H7.5M11.1667 4.5H9.5"
                  stroke="black"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
          ...
          <div className="flex flex-row overflow-x-auto overflow-y-hidden gap-[24px] pb-4 scrollbar-none snap-x snap-mandatory lg:grid lg:grid-cols-4 lg:overflow-x-visible lg:pb-0 lg:snap-none">
            {recommended?.products?.nodes
              ...
              .map((recProduct: any) => (
                <div
                  key={recProduct.id}
                  className="w-[280px] shrink-0 snap-start snap-always lg:w-auto lg:shrink-1 lg:snap-none text-start"
                >
                  <ProductItem product={recProduct} loading="lazy" />
                </div>
              ))}
          </div>
        </div>
...
function ProductGallery({images, product}: {images: any[]; product: any}) {
  ...
  const [zoomHoverProps, setZoomHoverProps] = useState({
    x: 0,
    y: 0,
    show: false,
  });
  ...
  const isVisibilityBlocked =
    !visibility.isActive ||
    product.tags?.some(
      (t: string) => t.toLowerCase().includes('season') || t.includes('موسم'),
    ) ||
  ...
  return (
    <div className="flex flex-col gap-4 relative w-full">
          ...
          >
            {images.map((img, i) => (
              <div
                ...
                className={`shrink-0 w-[calc(25%-9px)] aspect-square bg-[#f9f9f9] rounded-2xl border-2 transition-all cursor-pointer flex items-center justify-center overflow-hidden snap-start shadow-sm ${
                  i === activeIndex
      ...
      {mounted &&
        ...
        createPortal(
          <div
            className="fixed inset-0 z-[9999999] bg-white flex flex-col items-center justify-center animate-fade-in"
            dir="ltr"
          >
            {/* Close Button */}
            <button
      ...
      >
        {({option}) => {
          if (
            ...
            option.values[0].value === 'Default Title'
          ) {
            return null;
          }
          const optNameLower = option.name.toLowerCase();
          const isSize = optNameLower === 'size' || optNameLower === 'title';
          const isDenomination =
            optNameLower === 'denominations' ||
            optNameLower === 'denomination' ||
            optNameLower === 'value' ||
            optNameLower === 'amount';
                ...
                {option.values.map(({value, isAvailable, isActive, to}) => {
                  ...
                  const isMonetary =
                    ...
                    (value.toLowerCase().includes('sar') ||
                      value.includes('ر.س') ||
                      /^\d+(\.\d+)?$/.test(cleanNumStr));

                  const pillTextColor = isActive
                    ? 'text-white'
                    : 'text-[#255441]';
                  ...
                  if (!isEn) {
                    if (value.toLowerCase() === 'small') renderedValue = 'صغير';
                      ...
                      renderedValue = (
                        <span
                          className={`inline-flex items-center justify-center gap-1.5 font-bold ${pillTextColor}`}
                        >
                          <span>{formattedNum}</span>
                          <SaudiRiyalSymbol
                            className={`h-4 w-auto shrink-0 mb-0.5 ${isActive ? 'text-white fill-white' : 'text-[#255441]'}`}
                          />
                        </span>
                      );
                    } else {
                  ...
                  return (
                    <Link
                      ...
                      replace
                      to={variantUrl}
                      className={`h-[44px] px-[20px] rounded-[22px] text-[14px] font-bold border transition-all flex items-center justify-center min-w-[90px] shadow-sm ${
                        isActive
