      ...
      <div className="relative z-10 w-full max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12 flex items-center justify-between">
        ...
        <div className={`flex flex-col ${isEn ? 'items-start' : 'items-end'} gap-[8px]`}>
          <div className="flex items-center gap-2 md:gap-4" dir={isEn ? 'ltr' : 'rtl'}>
            <button onClick={() => window.history.back()} className={`flex items-center gap-[8px] bg-[#9FB7AE] hover:bg-[#8BA19C] text-[#234745] px-4 md:px-6 py-2.5 rounded-[25px] text-[12px] md:text-[16px] font-bold transition-all shrink-0 ${isEn ? 'font-en' : ''}`} style={isEn ? {} : { fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif" }} dir={isEn ? 'ltr' : 'rtl'}>
        ...
        <div className={`flex-1 ${isDesktop ? 'p-6' : 'overflow-y-auto p-6 pt-4'} flex flex-col`}>
            <div className="flex-1">
                ...
                {filters?.map((filter: any) => {
                    ...
                    if (lowerId.includes('availability') || lowerLabel === 'availability' || filter.label === 'التوفر' || filter.id === 'filter.p.tag' || filter.label === 'More filters' || filter.label === 'المزيد من الفلاتر') return null;
            ...
            <div className="mb-6 border-b border-gray-100 pb-6 mt-6">
                <button onClick={() => toggleSection('النوع الغذائي')} className="w-full flex items-center justify-between group">
                    <h3 className="text-[15px] font-black text-gray-800 tracking-wide">{isEn ? 'Dietary Preference' : 'النوع الغذائي'}</h3>
                    <svg className={`w-4 h-4 text-gray-400 transition-transform duration-300 ${openSections['النوع الغذائي'] !== false ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                </button>
                <div className={`mt-4 overflow-hidden transition-all duration-300 ${openSections['النوع الغذائي'] !== false ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className="flex flex-col gap-3">
                        {[
                          ...
                          { labelAr: 'قليل الدهون', labelEn: 'Low-Fat', tags: ['low-fat', 'low_fat', 'قليل الدهون', 'dietary:low-fat'] },
                            ...
                            const activeTag = item.tags.find(t => searchParams.getAll('filter.p.tag').includes(t));
