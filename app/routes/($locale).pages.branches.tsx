...
export default function BranchesPage() {
  ...
  const googleMapsKey =
    ...
    (typeof window !== 'undefined'
      ? (window as any).ENV?.PUBLIC_GOOGLE_MAPS_KEY
      : undefined);
  ...
  const {cityGroups, citiesList, totalCitiesCount} = useMemo(() => {
    ...
    const sortedCityNames = Object.keys(groups).sort(
  ...
  const dynamicDeliveryZones = useMemo(() => {
    const cityNames = Object.keys(cityGroups).sort(
      (a, b) => cityGroups[b].length - cityGroups[a].length,
    );
  ...
  useEffect(() => {
    ...
    if (!(window as any).google) {
      ...
      if (!document.getElementById(scriptId)) {
        ...
        script.onerror = () =>
          setMapError(
            'Failed to load Google Maps script. It might be blocked.',
          );
  ...
  return (
    <div
      className={`w-full bg-[#FAFAFA] min-h-screen ${fontClass}`}
      dir={isEn ? 'ltr' : 'rtl'}
    >
      {/* ─── HERO SECTION ─────────────────────────────────────────────────── */}
      <PageHeader
      ...
      >
        <p
          className="text-[#BBCFCD] text-[18px] mb-12 max-w-lg mx-auto mt-4"
          style={{fontFamily: fontFam2, lineHeight: '100%'}}
        >
          {isEn
            ? 'Search for the nearest branch or check delivery availability in your area'
            : 'ابحث عن أقرب فرع أو تحقق من توفر التوصيل لمنطقتك'}
        </p>

        {/* Stats */}
        <div
          className="flex justify-center items-center gap-4 lg:gap-16 mt-4 w-full"
        ...
        <div className="max-w-[1400px] mx-auto px-4 lg:px-6 flex flex-col lg:flex-row items-center justify-between gap-6">
          ...
          <div className="w-full lg:w-[450px] flex items-center bg-white border border-gray-300 rounded-full p-1 relative shrink-0">
            ...
            <input
              ...
              placeholder={
                isEn
                  ? 'Search for city or neighborhood...'
                  : 'إبحث عن مدينة أو حي...'
              }
            ...
            <button
              className="bg-[#234745] hover:bg-[#1a3533] text-white px-8 py-2 rounded-full text-sm font-bold transition-colors shrink-0"
              style={{fontFamily: fontFam}}
            >
              {isEn ? 'Search' : 'بحث'}
            </button>
      ...
      <div className="max-w-[1400px] mx-auto px-4 lg:px-6 py-6 !mb-4">
        <div className="flex flex-col lg:flex-row gap-6 h-auto lg:h-[800px]">
          ...
          <div className="w-full lg:w-[400px] flex flex-col h-[380px] lg:h-full bg-white border border-gray-300 rounded-[20px] p-4 shrink-0">
            <div
              className="mb-4 text-[#1a1a1a] font-bold text-[14px] flex items-center justify-between px-2"
              style={{fontFamily: fontFam}}
            >
              <span>
                {isEn
              ...
              {selectedCity !== 'all' && (
                <button
                  ...
                  className="text-xs text-[#234745] underline font-normal"
                >
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center mb-3">
                      <h3
                        className="font-bold text-[14px] text-[#234745]"
                        style={{fontFamily: fontFam}}
                      >
                        {!isEn &&
                        (branch.name_in_arabic?.value ||
                          branch.name_in_arabic ||
                          branch.metafields?.find(
          ...
          <div className="w-full bg-gray-200 rounded-[20px] overflow-hidden h-[420px] lg:h-full lg:flex-1 relative border border-gray-300 z-10 flex items-center justify-center">
            {mapError && (
              <div
                className="absolute inset-0 flex items-center justify-center bg-gray-100 z-20 p-6 text-center text-red-500 font-bold"
                style={{fontFamily: fontFam}}
              >
                {mapError}
              </div>
            ...
            {selectedBranch && (
              ...
              >
                <h4 className="font-bold text-[14px] text-[#1a1a1a] mb-1 text-start">
                  {!isEn &&
                  (selectedBranch.name_in_arabic?.value ||
                    selectedBranch.name_in_arabic ||
                    selectedBranch.metafields?.find(
                      (m: any) => m?.key === 'name_in_arabic',
                    )?.value)
                    ? selectedBranch.name_in_arabic?.value ||
                      selectedBranch.name_in_arabic ||
                ...
                <p className="text-[12px] text-gray-500 mb-4 text-start">
                  {selectedBranch.address?.address1 || ''}
                </p>
      ...
      <div className="max-w-[1400px] mx-auto px-4 lg:px-6 py-4 mb-20">
        <div className="flex flex-row justify-between !items-center mb-8 gap-4">
          <h2
            className="text-[20px] lg:text-[50px] font-bold !mb-0 text-[#234745]"
            style={{fontFamily: isEn ? fontFam : "'Bahij Janna', sans-serif"}}
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
          {dynamicDeliveryZones.map((zone) => (
            ...
            >
              <h3
                className="text-[20px] md:text-[30px] font-bold text-[#234745] mb-2 transition-transform"
                style={{
