...
export default function Register() {
  ...
  const showError = !!(errorToDisplay && (!hasEditSinceError || blockCooldown > 0));
        ...
        <div className="w-full lg:w-1/2 bg-white border border-[#BBCFCD]/50 rounded-[24px] flex flex-col items-center justify-center p-4 lg:p-12 relative shadow-sm">
          ...
          <div className="w-full bg-[#234745] text-white rounded-[28px] p-6 flex flex-col items-center justify-center text-center relative overflow-hidden mb-6 lg:hidden shadow-md">
            ...
            <div className="w-full flex justify-end mb-2">
              ...
              >
                ...
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className={!isEn ? 'rotate-180' : ''}>
                  <path d="M19 12H5M5 12L12 19M5 12L12 5" stroke="#FEF8EB" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </Link>
            </div>
            ...
            {/* Subtitle */}
            <p className="text-[#FEF8EB]/90 text-[13px] sm:text-[14px] font-medium leading-[22px] max-w-[280px] mt-2" style={{ fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif" }}>
          ...
          <div className="w-full flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center gap-6">
              ...
              {step === 'input' ? (
                <>
                  <Form method="POST" className="w-full flex flex-col gap-4 w-full">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <input
                        ...
                        required
                      />
                      <label htmlFor="terms" className={`text-[#7D7D7D] text-[12px] font-medium flex gap-1 ${isEn ? 'flex-row' : 'flex-row-reverse'}`} style={{ fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif" }}>
                        <span className="text-[#E55C5C]">*</span>
                        <span>
                          {isEn ? 'I agree to the ' : 'أوافق على '}
                          <span className="font-bold border-b border-[#171717] !text-[#234745]">{isEn ? 'Terms of Use' : 'شروط الاستخدام'}</span>
                          {isEn ? ' and ' : ' '}
                          <span className="font-bold border-b border-[#171717] !text-[#234745]">{isEn ? 'Privacy Policy' : 'وسياسة الخصوصية'}</span>
                        </span>
                      </label>
                    </div>

                    {showError && errorToDisplay && <p className="text-red-500 text-sm text-center mt-1">{errorToDisplay}</p>}

                    {/* Submit Button */}
                    <button
                      type="submit"
                      disabled={isLoading || formData.phone.length < 9 || !formData.fullName || !formData.termsAccepted || blockCooldown > 0}
                ...
                <Form method="POST" className="w-full flex flex-col gap-6">
                  ...
                  {showError && errorToDisplay && blockCooldown > 0 && (
                    <div className="w-full border border-[#EAD6BA] bg-[#FCF7ED] rounded-[16px] p-5 flex flex-col gap-2 text-[#8B6D43] text-sm relative" dir={isEn ? 'ltr' : 'rtl'}>
                  ...
                  <div className="flex flex-col gap-2 w-full items-center">
                    <div className="text-[14px] font-medium text-[#707070] mb-2 flex items-center justify-center gap-1 flex-wrap" style={{ fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif" }}>
                      ...
                      <span className="font-bold text-[#171717]" dir="ltr">{formData.countryCode} {formData.phone}</span>
                      <button type="button" onClick={() => setStep('input')} className="text-[#234745] font-bold underline hover:text-[#1a3533] ml-1">
                        {isEn ? 'Edit' : 'تعديل'}
                      </button>
                    </div>
                  ...
                  {showError && errorToDisplay && blockCooldown <= 0 && (
                    <div className="w-full border border-[#F38C8C] bg-[#FFF5F5] rounded-[12px] py-3 px-4 flex items-center gap-3 text-[#E55C5C] text-sm font-semibold justify-center mt-1" dir={isEn ? 'ltr' : 'rtl'}>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
                        <circle cx="12" cy="12" r="10" />
                  ...
                  <button
                    type="submit"
                    disabled={isLoading || otpValue.some(v => !v) || blockCooldown > 0}
                    className="w-full bg-[#234745] text-[#FEF8EB] font-bold text-[16px] rounded-[25px] h-[48px] flex items-center justify-center hover:bg-[#1a3533] transition-colors disabled:opacity-70"
                    style={{ fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif" }}
                  >
