...
export const meta: MetaFunction<typeof loader> = ({matches}) => {
  return [{title: pageTitle(matches, 'Create Account', 'إنشاء حساب')}];
};
...
export async function loader({context, request}: LoaderFunctionArgs) {
  ...
  const blockUntil = await context.session.get('registerOtpBlockUntil');
...
export async function action({request, context}: ActionFunctionArgs) {
        ...
        if (email) {
          ...
          if (emailCheckRes.ok) {
            ...
            if (checkData.customers?.length > 0) {
              console.warn(
                '[Register] Email already exists in Shopify before sending OTP:',
                email,
              );
              return data({
                error:
  ...
  if (intent === 'register-with-otp') {
      ...
      return data({
        error:
          lang === 'en'
            ...
            : 'جلسة غير صالحة. يرجى المحاولة مرة أخرى.',
      });
    }

    // Extract Customer Details
    const accountType = String(form.get('accountType') || 'individual');
    const firstNameInput = String(form.get('firstName') || '').trim();
    const lastNameInput = String(form.get('lastName') || '').trim();
      ...
      if (adminToken) {
        if (email) {
          ...
          if (emailCheckRes.ok) {
            ...
            if (checkData.customers?.length > 0) {
              console.warn(
                '[Register] Email already exists in Shopify:',
                email,
              );
              return data({
                error:
    ...
    try {
      ...
      try {
        const customRegister = await api.register(
          registerPayload,
          idempotencyKey,
        );
      ...
      try {
        ...
        console.warn(
          '[Register] Shopify customer access token creation failed (non-fatal):',
          shopifyErr,
        );
      }
      ...
      if (!token) {
        console.warn(
          '[Register] Storefront token creation failed, attempting Admin API customer sync fallback',
        );
        try {
          const {getAdminToken} = await import('~/lib/shopify-admin.server');
          const adminToken = await getAdminToken(env);
          ...
          if (adminToken) {
            ...
            if (adminResponse.ok) {
              ...
              console.log(
                '[Register] Fallback customer created successfully in Shopify Admin:',
                adminData.customer?.id,
              );
              if (adminData.customer?.id) {
                session.set(
                  'loginCustomerId',
                  `gid://shopify/Customer/${adminData.customer.id}`,
                );
              }
              ...
              token =
                tokenResponse.customerAccessTokenCreate?.customerAccessToken ||
                null;
            } else {
              const errBody = (await adminResponse.json()) as any;
              console.error(
                '[Register] Fallback Shopify Admin customer creation failed:',
          ...
          console.error(
            '[Register] Fallback Shopify Admin sync error:',
            fallbackErr,
          );
        }
      }
      ...
      const targetRedirect =
        selectedLanguage === 'en' ? `/en${cleanRedirect}` : cleanRedirect;
...
export default function Register() {
  ...
  const showError = !!(
    errorToDisplay &&
    (!hasEditSinceError || blockCooldown > 0)
  );
        ...
        <div className="w-full lg:w-1/2 bg-white border border-[#BBCFCD]/50 rounded-[24px] overflow-hidden flex flex-col items-center justify-center p-0 lg:p-12 relative shadow-sm">
          ...
          <div className="w-full bg-[#234745] text-white rounded-b-[24px] p-6 flex flex-col items-center justify-center text-center relative overflow-hidden lg:hidden shadow-md">
            ...
            <div className="w-full flex justify-end mb-2">
              <Link
                to={isEn ? '/en' : '/'}
                className="bg-[#BBCFCD]/20 hover:bg-[#BBCFCD]/30 text-[#FEF8EB] rounded-full px-4 py-1.5 text-[13px] font-bold flex items-center gap-2 transition-colors border border-white/10"
                style={{
            ...
            {/* Subtitle */}
            <p
              className="text-[#FEF8EB]/90 text-[13px] sm:text-[14px] font-medium leading-[22px] max-w-[280px] mt-2"
              style={{
                fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
              }}
            >
          ...
          <div className="w-full flex flex-col items-center p-6 lg:p-0">
            ...
            <div className="flex flex-col items-center mb-6 gap-2 w-full border-b border-[#BBCFCD]/50 pb-6">
              <h1
                className="text-[26px] font-bold text-[#171717] flex items-center gap-2"
                style={{
              ...
              >
                {isEn
                  ? 'Enter the information below and start the wonderful Saadeddin experience'
                  : 'أدخل المعلومات أدناه وابدأ تجربة سعد الدين الرائعة'}
              </p>
            </div>
            ...
            <div className="w-full flex flex-col items-center gap-6">
              ...
              <div className="flex w-full gap-4 h-[48px]">
                <Link
                  to={
                    isEn
                      ? `/en/account/login${redirectTo ? `?redirectTo=${encodeURIComponent(redirectTo)}` : ''}`
              ...
              {step === 'input' ? (
                  ...
                  >
                    ...
                    <div className="flex w-full gap-4 h-[40px] mb-2">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setFormData({...formData, accountType: 'company'})
                        }
                      ...
                      <>
                        <div className="flex flex-col gap-2 w-full">
                          ...
                          <input
                            ...
                            type="text"
                            placeholder={
                              isEn ? 'Company LLC' : 'شركة التقنية المحدودة'
                            }
                            className="w-full bg-white border border-[#BBCFCD] rounded-[12px] px-4 py-3 h-[48px] focus:border-[#234745] outline-none text-[#171717] font-medium text-[14px] placeholder:text-[#BBCFCD] transition-colors"
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <input
                        ...
                        className="w-4 h-4 rounded border-[#BBCFCD] text-[#234745] focus:ring-[#234745] cursor-pointer"
                        required
                      />
                      <label
                        htmlFor="terms"
                        className={`text-[#7D7D7D] text-[12px] font-medium flex gap-1 ${isEn ? 'flex-row' : 'flex-row-reverse'}`}
                        style={{
                          fontFamily:
                            "'EnglishDigits', 'GE Dinar One', sans-serif",
                        }}
                      >
                        ...
                        <span>
                          ...
                          <span className="font-bold border-b border-[#171717] !text-[#234745]">
                            {isEn ? 'Terms of Use' : 'شروط الاستخدام'}
                          </span>
                          {isEn ? ' and ' : ' '}
                          <span className="font-bold border-b border-[#171717] !text-[#234745]">
                            {isEn ? 'Privacy Policy' : 'وسياسة الخصوصية'}
                          </span>
                        </span>
                      </label>
                    </div>
                ...
                <Form method="POST" className="w-full flex flex-col gap-6">
                  ...
                  {showError && errorToDisplay && blockCooldown > 0 && (
                    <div
                      className="w-full border border-[#EAD6BA] bg-[#FCF7ED] rounded-[16px] p-5 flex flex-col gap-2 text-[#8B6D43] text-sm relative"
                      dir={isEn ? 'ltr' : 'rtl'}
                    >
                  ...
                  <div className="flex flex-col items-center gap-1 mt-2">
                    {blockCooldown > 0 ? null : resendCooldown > 0 ? (
                      <p
                        className="text-sm font-medium text-[#A19F9F]"
                        style={{
                          fontFamily:
                            "'EnglishDigits', 'GE Dinar One', sans-serif",
                        }}
                  ...
                  <button
                    type="button"
                    className="text-[#9FB7AE] hover:underline text-sm font-medium mx-auto"
