  ...
  return data({
    locations: locations.map((loc: any) => {
      return {
        ...
        name: loc.name,
        delivery_time:
          loc.metafields.nodes.find((m: any) => m.key === 'delivery_time')
            ?.value || '30-45 mins',
        delivery_fee:
          loc.metafields.nodes.find((m: any) => m.key === 'delivery_fee')
            ?.value || '0',
        threshold:
        ...
        hours_from:
          loc.metafields.nodes.find((m: any) => m.key === 'working_hours_from')
            ?.value || '8:00 AM',
        hours_to:
          loc.metafields.nodes.find((m: any) => m.key === 'working_hours_to')
            ?.value || '11:00 PM',
        hours_from_shift2:
          loc.metafields.nodes.find(
        ...
        working_days: (() => {
          const val = loc.metafields.nodes.find(
...
export async function action({request, context}: ActionFunctionArgs) {
  ...
  if (intent === 'bulk-update-timings') {
    ...
    const updateDefaultTimings =
      formData.get('updateDefaultTimings') === 'true';
    ...
    const locationNodes = (jsonLocs?.data?.locations?.nodes || []).filter(
      (loc: any) => !excludeBranches.includes(loc.id),
    );

    if (locationNodes.length === 0) {
      return data(
        {error: 'No branch locations found to update.'},
        {status: 400},
      );
    }
    ...
    for (const loc of locationNodes) {
      ...
      for (const day of updateDays) {
        ...
        if (hoursFrom)
          metafieldsPayload.push({
            ownerId: loc.id,
            namespace: 'custom',
            key: `${lowerDay}_working_hours_from`,
            value: hoursFrom,
            type: 'single_line_text_field',
          });
        if (hoursTo)
          metafieldsPayload.push({
            ownerId: loc.id,
            namespace: 'custom',
            key: `${lowerDay}_working_hours_to`,
            value: hoursTo,
            type: 'single_line_text_field',
          });
      }
      ...
      if (doUpdateWorkingDays) {
        metafieldsPayload.push({
          ...
          namespace: 'custom',
          key: 'working_days',
          value: workingDaysJson,
          type: 'json',
        });
      }
    }

    const filteredPayload = metafieldsPayload.filter(
    ...
    return data({
      success: true,
      message:
        'Successfully updated timings and days for all branches in bulk!',
    });
  }
  ...
  const variables = {
    metafields: [
      ...
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'friday_working_hours_from',
        value: fridayHoursFrom || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'friday_working_hours_to',
        value: fridayHoursTo || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'saturday_working_hours_from',
        value: saturdayHoursFrom || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'saturday_working_hours_to',
        value: saturdayHoursTo || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'sunday_working_hours_from',
        value: sundayHoursFrom || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'sunday_working_hours_to',
        value: sundayHoursTo || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'monday_working_hours_from',
        value: mondayHoursFrom || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'monday_working_hours_to',
        value: mondayHoursTo || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'tuesday_working_hours_from',
        value: tuesdayHoursFrom || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'tuesday_working_hours_to',
        value: tuesdayHoursTo || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'wednesday_working_hours_from',
        value: wednesdayHoursFrom || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'wednesday_working_hours_to',
        value: wednesdayHoursTo || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'thursday_working_hours_from',
        value: thursdayHoursFrom || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
        namespace: 'custom',
        key: 'thursday_working_hours_to',
        value: thursdayHoursTo || '',
        type: 'single_line_text_field',
      },
      {
        ownerId: locationId,
    ...
    <div className="account-dashboard-content" dir={isEn ? 'ltr' : 'rtl'}>
      <div className="mb-10">
        ...
        <p className="text-gray-500 font-medium">
          {isEn
            ? 'Manage delivery times, fees, and operating days for all branches'
            : 'إدارة أوقات ورسوم وأيام العمل لجميع الفروع'}
        </p>
      </div>
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h3 className="text-xl font-black text-[#234745] flex items-center gap-2">
              ...
              {isEn
                ? 'Bulk Timings & Holiday Planner (Ramadan / Eid)'
                : 'تخطيط أوقات العمل الجماعي (رمضان / الأعياد)'}
            </h3>
            <p className="text-xs text-gray-500 mt-1 font-medium">
              {isEn
                ? 'Apply timings and operating days to all branches at once'
                : 'تطبيق أوقات وأيام العمل لجميع الفروع دفعة واحدة'}
            </p>
          </div>
        ...
        <Form method="POST" className="space-y-5">
          ...
          <div className="luxury-field border-t border-[#f0ece8] pt-3.5">
            <label className="luxury-label font-black text-gray-800 mb-2">
              {isEn
                ? 'Timings to Update (Check to apply, uncheck to exclude)'
                : 'الأوقات المراد تحديثها (اختر للتطبيق، ألغِ للتمرير والاستبعاد)'}
            </label>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mt-1.5">
              <label className="flex items-center gap-2 text-xs font-bold cursor-pointer text-gray-700 bg-white p-2 rounded-xl border border-gray-200 hover:border-[#d4a06a] transition-all">
                ...
                />
                <span>
                  {isEn ? 'Default/General Hours' : 'الأوقات العامة/الافتراضية'}
                </span>
              </label>
              {daysOfWeek.map((day) => (
                <label
                  key={day.key}
                  className="flex items-center gap-2 text-xs font-bold cursor-pointer text-gray-700 bg-white p-2 rounded-xl border border-gray-200 hover:border-[#d4a06a] transition-all"
                >
                  <input
                    type="checkbox"
                    name="updateDays"
                  ...
                  />
                  <span>
                    {isEn ? `${day.labelEn} Overrides` : `أوقات ${day.labelAr}`}
                  </span>
                </label>
              ))}
            </div>
          ...
          <div className="luxury-field border-t border-[#f0ece8] pt-3.5">
            <div className="flex items-center justify-between mb-2">
              <label className="luxury-label mb-0">
                {isEn
                  ? 'Update Operating Days (Optional)'
                  : 'تحديث أيام العمل الفعلية للفروع (اختياري)'}
              </label>
              <label className="flex items-center gap-1.5 text-xs text-[#234745] font-black cursor-pointer bg-white border border-[#234745]/20 px-2.5 py-1 rounded-full hover:bg-gray-50">
                ...
                <span>
                  {isEn
                    ? 'Apply operating days update'
                    : 'تطبيق تحديث أيام العمل'}
                </span>
              </label>
            </div>
            ...
            {updateWorkingDays && (
              <div className="flex flex-wrap gap-2.5 mt-2 animate-fade-in">
                {daysOfWeek.map((day) => (
                  <label
                    key={day.key}
                    className="flex items-center gap-1.5 text-xs text-gray-700 cursor-pointer font-bold bg-white px-3 py-1.5 rounded-xl border border-gray-200 hover:border-[#d4a06a]"
                  >
                    <input
                      type="checkbox"
                      name="workingDays"
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 mt-3 p-3 bg-gray-50 border border-gray-200 rounded-xl animate-fade-in">
                {locations.map((loc: any) => {
                  ...
                  return (
                    <label
                      key={loc.id}
                      className={`flex items-center gap-2 text-xs font-bold cursor-pointer p-2 rounded-xl border transition-all ${isExcluded ? 'bg-red-50 border-red-200 text-red-700 font-extrabold shadow-sm' : 'bg-white border-gray-200 text-gray-700 hover:border-[#d4a06a]'}`}
                    >
                      <input
                        type="checkbox"
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            ...
                            setExcludedBranchIds(
                              excludedBranchIds.filter((id) => id !== loc.id),
                            );
                          }
                        }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {locations.map((loc: any) => {
          ...
          if (loc.sunday_hours_from || loc.sunday_hours_to)
            activeOverrides.push(isEn ? 'Sun' : 'الأحد');
          if (loc.monday_hours_from || loc.monday_hours_to)
            activeOverrides.push(isEn ? 'Mon' : 'الاثنين');
          if (loc.tuesday_hours_from || loc.tuesday_hours_to)
            activeOverrides.push(isEn ? 'Tue' : 'الثلاثاء');
          if (loc.wednesday_hours_from || loc.wednesday_hours_to)
            activeOverrides.push(isEn ? 'Wed' : 'الأربعاء');
          if (loc.thursday_hours_from || loc.thursday_hours_to)
            activeOverrides.push(isEn ? 'Thu' : 'الخميس');
          if (loc.friday_hours_from || loc.friday_hours_to)
            activeOverrides.push(isEn ? 'Fri' : 'الجمعة');
          if (loc.saturday_hours_from || loc.saturday_hours_to)
            activeOverrides.push(isEn ? 'Sat' : 'السبت');

          return (
            <div
              key={loc.id}
              className="luxury-card p-6 border border-[#e2dcd5] hover:border-[#d4a06a] transition-all bg-white rounded-2xl shadow-sm"
            >
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h3 className="text-xl font-bold text-[#234745]">
                    {loc.name}
                  </h3>
                  {activeOverrides.length > 0 && (
                    <div className="flex flex-wrap items-center gap-1.5 mt-2 animate-fade-in">
                      ...
                      {activeOverrides.map((day) => (
                        <span
                          key={day}
                          className="bg-[#f0ece8] text-[#d4a06a] text-[9.5px] font-black px-2 py-0.5 rounded-full shadow-sm"
                        >
                          {day}
                        </span>
                      ))}
              ...
              <Form method="POST" className="space-y-4 mt-4">
                ...
                <div className="luxury-field">
                  <label className="luxury-label">
                    {isEn ? 'Estimated Delivery Time' : 'وقت التوصيل المتوقع'}
                  </label>
                  <input
                    name="deliveryTime"
                ...
                <details className="border border-[#eee] rounded-xl p-3 bg-gray-50 mt-4">
                  <summary className="cursor-pointer font-bold text-xs text-[#234745] hover:text-[#d4a06a] outline-none select-none">
                    {isEn
                      ? '⚙️ Day-by-Day Custom Timings (Optional)'
                      : '⚙️ أوقات عمل مخصصة لكل يوم (اختياري)'}
                  </summary>
                  ...
                  <div className="mt-4 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Sunday Open From' : 'الأحد يفتح من'}
                        </label>
                        <input
                          name="sundayHoursFrom"
                          defaultValue={loc.sunday_hours_from}
                          className="luxury-input-field text-xs"
                          placeholder="8:00 AM"
                        />
                      </div>
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Sunday Close At' : 'الأحد يغلق في'}
                        </label>
                        <input
                          name="sundayHoursTo"
                          defaultValue={loc.sunday_hours_to}
                          className="luxury-input-field text-xs"
                          placeholder="11:00 PM"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Monday Open From' : 'الاثنين يفتح من'}
                        </label>
                        <input
                          name="mondayHoursFrom"
                          defaultValue={loc.monday_hours_from}
                          className="luxury-input-field text-xs"
                          placeholder="8:00 AM"
                        />
                      </div>
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Monday Close At' : 'الاثنين يغلق في'}
                        </label>
                        <input
                          name="mondayHoursTo"
                          defaultValue={loc.monday_hours_to}
                          className="luxury-input-field text-xs"
                          placeholder="11:00 PM"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Tuesday Open From' : 'الثلاثاء يفتح من'}
                        </label>
                        <input
                          name="tuesdayHoursFrom"
                          defaultValue={loc.tuesday_hours_from}
                          className="luxury-input-field text-xs"
                          placeholder="8:00 AM"
                        />
                      </div>
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Tuesday Close At' : 'الثلاثاء يغلق في'}
                        </label>
                        <input
                          name="tuesdayHoursTo"
                          defaultValue={loc.tuesday_hours_to}
                          className="luxury-input-field text-xs"
                          placeholder="11:00 PM"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Wednesday Open From' : 'الأربعاء يفتح من'}
                        </label>
                        <input
                          name="wednesdayHoursFrom"
                          defaultValue={loc.wednesday_hours_from}
                          className="luxury-input-field text-xs"
                          placeholder="8:00 AM"
                        />
                      </div>
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Wednesday Close At' : 'الأربعاء يغلق في'}
                        </label>
                        <input
                          name="wednesdayHoursTo"
                          defaultValue={loc.wednesday_hours_to}
                          className="luxury-input-field text-xs"
                          placeholder="11:00 PM"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Thursday Open From' : 'الخميس يفتح من'}
                        </label>
                        <input
                          name="thursdayHoursFrom"
                          defaultValue={loc.thursday_hours_from}
                          className="luxury-input-field text-xs"
                          placeholder="8:00 AM"
                        />
                      </div>
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Thursday Close At' : 'الخميس يغلق في'}
                        </label>
                        <input
                          name="thursdayHoursTo"
                          defaultValue={loc.thursday_hours_to}
                          className="luxury-input-field text-xs"
                          placeholder="11:00 PM"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Friday Open From' : 'الجمعة يفتح من'}
                        </label>
                        <input
                      ...
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Friday Close At' : 'الجمعة يغلق في'}
                        </label>
                        <input
                          name="fridayHoursTo"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Saturday Open From' : 'السبت يفتح من'}
                        </label>
                        <input
                      ...
                      <div className="luxury-field">
                        <label className="luxury-label">
                          {isEn ? 'Saturday Close At' : 'السبت يغلق في'}
                        </label>
                        <input
                ...
                <div className="luxury-field">
                  ...
                  <div className="grid grid-cols-4 gap-2 mt-1">
                    {daysOfWeek.map((day) => {
                      const isChecked = Array.isArray(loc.working_days)
                        ? loc.working_days.includes(day.key)
                        : true;
                      return (
                        <label
                          key={day.key}
                          className="flex items-center gap-1.5 text-xs text-gray-700 cursor-pointer font-medium"
                        >
                          <input
                            type="checkbox"
                            name="workingDays"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="luxury-field">
                    <label className="luxury-label">
                      {isEn ? 'Threshold' : 'حد التوصيل المجاني'}
                    </label>
                    <input
                      type="number"
                      name="threshold"
