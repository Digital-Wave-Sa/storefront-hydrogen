                    ...
                    <div className="w-full lg:w-[380px] shrink-0 flex flex-col gap-6 order-2">
                        <div className="bg-white rounded-[12px] border border-[#EBEBEB] p-6">
                            ...
                            <div className="flex justify-between items-end mb-6">
                                ...
                                <span className="text-[24px] font-black text-[#1A1A1A] flex items-center gap-2" dir="ltr"><CurrencyIcon className="h-5 w-auto" /> {forceEnNums(orderData.summary.total)}</span>
                            </div>
                            ...
                            <div className="flex flex-col gap-4 mb-8">
                                <div className="flex justify-between items-center text-[14px]">
                                    ...
                                    <span className="font-bold text-[#1A1A1A]">{orderData.address}</span>
                                </div>
                                <div className="flex justify-between items-center text-[14px]">
                                    <span className="text-[#8B8B8B]">{isEn ? 'Payment Method' : 'طريقة الدفع'}</span>
                                    <span className="font-bold text-[#1A1A1A]">{orderData.paymentMethod}</span>
                                </div>
                            </div>
                    ...
                    <div className="flex-1 w-full flex flex-col gap-6 order-1">
                        ...
                        <div className="bg-white rounded-[12px] border border-[#EBEBEB] p-6 lg:p-8">
                            ...
                            <div className="relative flex flex-col gap-8">
                                {/* Vertical Line */}
                                <div className={`absolute top-4 bottom-4 ${isEn ? 'left-4' : 'right-4'} w-[2px] bg-[#EBEBEB]`} />
                                ...
                                {(() => {
                                    ...
                                    const timeStr = orderData.date.split('،')[1] || orderData.date.split(',')[1] || '';

                                    const stages = [
                                        { id: 1, en: 'Order Received', ar: 'تم استلام الطلب', descEn: forceEnNums(`Order successfully received ${timeStr}`), descAr: forceEnNums(`تم استلام طلبك بنجاح ${timeStr}`) },
                                        { id: 2, en: 'Order Processed', ar: 'تم معالجة الطلب', descEn: currentStep >= 2 ? 'Order has been confirmed' : '', descAr: currentStep >= 2 ? 'تم تأكيد طلبك' : '' },
                                        { id: 3, en: 'Preparing', ar: 'جاري التجهيز', descEn: '', descAr: '' },
                                        { id: 4, en: 'On the Way', ar: 'في الطريق إليك', descEn: '', descAr: '' },
                                        { id: 5, en: 'Delivered', ar: 'تم التسليم', descEn: '', descAr: '' },
                                    ];

                                    return stages.map((stage) => {
                                        const isCompleted = currentStep >= stage.id;
                                        const isCurrent = currentStep === stage.id;
                                        const isPending = currentStep > 0 && currentStep < stage.id;
                                        ...
                                        return (
                                            <div key={stage.id} className="relative flex gap-6 items-start">
                                                {isCompleted ? (
                                                    ...
                                                    <div className={`w-8 h-8 rounded-full border-[1.5px] shrink-0 flex items-center justify-center relative z-10 ring-4 ring-white font-en text-[14px] font-bold ${isCurrent ? 'bg-[#234745] border-[#234745] text-white' : 'bg-white border-[#EBEBEB] text-[#8B8B8B]'}`}>
                                                        {isEn ? stage.id : ['١','٢','٣','٤','٥'][stage.id - 1]}
                                                    </div>
                                                )}
                                                <div className="flex flex-col pt-1">
                                                    <h3 className={`text-[16px] font-bold ${isCompleted || isCurrent ? 'text-[#1A1A1A]' : 'text-[#8B8B8B]'}`}>{isEn ? stage.en : stage.ar}</h3>
                                                    {(stage.descEn || stage.descAr) && (
                                                        <span className="text-[13px] text-[#8B8B8B] mt-1">{isEn ? stage.descEn : stage.descAr}</span>
                                                    )}
                                                </div>
                                            </div>
                                        );
                        ...
                        {/* Order Notifications */}
                        <div className="bg-white rounded-[12px] border border-[#EBEBEB] p-6 lg:p-8">
                            <h2 className="text-[20px] font-black text-[#1A1A1A] mb-6">{isEn ? 'Order Notifications' : 'إشعارات الطلب'}</h2>
                            
                            <div className="flex flex-col gap-6">
                                <div className="flex justify-between items-center">
                                    <span className="text-[16px] font-bold text-[#1A1A1A]">{isEn ? 'SMS Messages' : 'رسائل SMS'}</span>
                                    <button onClick={() => handleToggle('sms')} className={`w-[48px] h-[26px] rounded-full transition-colors relative flex items-center px-1 ${toggles.sms ? 'bg-[#234745]' : 'bg-[#EBEBEB]'}`}>
                                        <div className={`w-[20px] h-[20px] rounded-full bg-white shadow-sm transition-transform ${toggles.sms ? (isEn ? 'translate-x-[20px]' : '-translate-x-[20px]') : 'translate-x-0'}`} />
                                    </button>
                                </div>
                                <div className="w-full h-[1px] bg-[#F5F5F5]"></div>
                                
                                <div className="flex justify-between items-center">
                                    <span className="text-[16px] font-bold text-[#1A1A1A]">{isEn ? 'WhatsApp' : 'WhatsApp'}</span>
                                    <button onClick={() => handleToggle('whatsapp')} className={`w-[48px] h-[26px] rounded-full transition-colors relative flex items-center px-1 ${toggles.whatsapp ? 'bg-[#234745]' : 'bg-[#EBEBEB]'}`}>
                                        <div className={`w-[20px] h-[20px] rounded-full bg-white shadow-sm transition-transform ${toggles.whatsapp ? (isEn ? 'translate-x-[20px]' : '-translate-x-[20px]') : 'translate-x-0'}`} />
                                    </button>
                                </div>
                                <div className="w-full h-[1px] bg-[#F5F5F5]"></div>

                                <div className="flex justify-between items-center">
                                    <span className="text-[16px] font-bold text-[#1A1A1A]">{isEn ? 'Email' : 'البريد الإلكتروني'}</span>
                                    <button onClick={() => handleToggle('email')} className={`w-[48px] h-[26px] rounded-full transition-colors relative flex items-center px-1 ${toggles.email ? 'bg-[#234745]' : 'bg-[#EBEBEB]'}`}>
                                        <div className={`w-[20px] h-[20px] rounded-full bg-white shadow-sm transition-transform ${toggles.email ? (isEn ? 'translate-x-[20px]' : '-translate-x-[20px]') : 'translate-x-0'}`} />
                                    </button>
                                </div>
                                <div className="w-full h-[1px] bg-[#F5F5F5]"></div>

                                <div className="flex justify-between items-center">
                                    <span className="text-[16px] font-bold text-[#1A1A1A]">{isEn ? 'App Notifications' : 'إشعارات التطبيق'}</span>
                                    <button onClick={() => handleToggle('app')} className={`w-[48px] h-[26px] rounded-full transition-colors relative flex items-center px-1 ${toggles.app ? 'bg-[#234745]' : 'bg-[#EBEBEB]'}`}>
                                        <div className={`w-[20px] h-[20px] rounded-full bg-white shadow-sm transition-transform ${toggles.app ? (isEn ? 'translate-x-[20px]' : '-translate-x-[20px]') : 'translate-x-0'}`} />
                                    </button>
                                </div>
                            </div>
                        </div>
