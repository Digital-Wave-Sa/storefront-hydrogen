...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  if (intent === 'send-otp') {
    ...
    // Cooldown Throttle Check (60 seconds to match backend rate limit)
  ...
  if (intent === 'register-with-otp') {
    ...
    try {
      ...
      try {
        ...
        const rawError = verifyErr.message || (lang === 'en' ? 'Incorrect verification code.' : 'رمز التحقق غير صحيح.');
        return data({
          error: formatOtpError(rawError, lang)
        });
      }
          ...
          <div className="w-full flex flex-col items-center">
            ...
            <div className="w-full flex flex-col items-center gap-6">
              ...
              <div className="flex w-full gap-4 h-[48px]">
                <Link to={isEn ? "/en/account/login" : "/account/login"} className="flex-1 flex items-center justify-center bg-white border border-[#234745] text-[#234745] rounded-[25px] font-bold text-[16px] hover:bg-[#234745]/5 transition-colors" style={{ fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif" }}>
                      ...
                      {otpRefs.map((ref, i) => (
                        <input
                          ...
                          className={`w-14 h-14 text-center border rounded-[12px] text-2xl font-bold outline-none transition-colors ${
                            (fetcher.data?.error || actionData?.error) && !hasEditSinceError
