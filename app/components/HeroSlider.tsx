  ...
  const dynamicSlides = React.useMemo(() => {
    ...
    const parsed = rawNodes
      ...
      .map((node: any, idx: number) => {
        ...
        const badgeAr = fixMojibake(getVal('badge_text_arabic', 'badge_ar', 'badge_text_ar', 'badge', 'tag', 'badge_text') || defaultFallback.badge.ar);
        const badgeEn = fixMojibake(getVal('badge_text_english', 'badge_en', 'badge_text_en', 'badge', 'tag', 'badge_text') || defaultFallback.badge.en);
        const titleAr = fixMojibake(getVal('title_arabic', 'title_ar', 'title', 'heading_ar') || defaultFallback.title.ar);
        const titleEn = fixMojibake(getVal('title_english', 'title_en', 'title', 'heading_en') || defaultFallback.title.en);
        const subtitleAr = fixMojibake(getVal('subtitle_arabic', 'subtitle_ar', 'subtitle', 'description_ar') || defaultFallback.subtitle.ar);
        const subtitleEn = fixMojibake(getVal('subtitle_english', 'subtitle_en', 'subtitle', 'description_en') || defaultFallback.subtitle.en);
        const btnTextAr = fixMojibake(getVal('button_text_arabic', 'button_text_ar', 'button_text', 'cta_text_ar') || defaultFallback.buttons[0].text.ar);
        const btnTextEn = fixMojibake(getVal('button_text_english', 'button_text_en', 'button_text', 'cta_text_en') || defaultFallback.buttons[0].text.en);
