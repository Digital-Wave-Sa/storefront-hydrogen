...
async function uploadImageToShopify(
  ...
  try {
    ...
    const stagedUploadRes = (await adminApiQuery(
      shopDomain,
      token,
    ...
    if (
      stagedUploadRes.errors ||
      stagedUploadRes.data?.stagedUploadsCreate?.userErrors?.length
    ) {
      console.error(
        '[Staged Upload] GraphQL Error:',
        JSON.stringify(stagedUploadRes),
      );
    }

    const target =
      stagedUploadRes?.data?.stagedUploadsCreate?.stagedTargets?.[0];
    if (!target) {
    ...
    if (!uploadRes.ok) {
      console.error(
        '[Staged Upload] POST to target.url failed with status:',
        uploadRes.status,
    ...
    const fileCreateRes = (await adminApiQuery(
      shopDomain,
      token,
      fileCreateMutation,
      {
    ...
    if (
      fileCreateRes.errors ||
      fileCreateRes.data?.fileCreate?.userErrors?.length
    ) {
      console.error(
        '[Staged Upload] fileCreate GraphQL Error:',
        JSON.stringify(fileCreateRes),
      );
    }
    ...
    const finalUrlOrId =
      fileObj?.image?.url || fileObj?.url || fileObj?.id || null;

    if (!finalUrlOrId) {
      console.error(
        '[Staged Upload] fileObj missing url/id:',
  ...
  try {
    ...
    const customAttributes = [
      ...
      ...(messageFont
        ...
        : []),
      ...(messageColor
        ? [{key: isEn ? 'Message Color' : 'لون الرسالة', value: messageColor}]
        : []),
    ];
    ...
    if (uploadedImage && uploadedImage.startsWith('data:image')) {
      const shopifyFileUrl = await uploadImageToShopify(
        shopDomain,
        token,
        uploadedImage,
      );
    ...
    if (cakePreviewImage && cakePreviewImage.startsWith('data:image')) {
      const shopifyPreviewUrl = await uploadImageToShopify(
        shopDomain,
        token,
        cakePreviewImage,
      );
    ...
    try {
      ...
      `;
      const findVariantRes = (await adminApiQuery(
        shopDomain,
        token,
        findVariantQuery,
        {
      ...
      if (resolvedVar?.id) {
        ...
        console.log(
          `[Custom Cake] Resolved catalog variant ID ${variantId} for SKU ${closestMapping.sku}`,
        );
      } else {
        console.warn(
          `[Custom Cake] No catalog variant found for SKU ${closestMapping.sku}`,
        );
      }
    } catch (variantErr) {
      console.error(
        `[Custom Cake] Failed to resolve variant for SKU ${closestMapping.sku}:`,
        variantErr,
      );
    }
      ...
      lineItems: [
        {
          ...
          customAttributes: [
            ...
            ...(finalImageAttr
              ? [
                  {
                    key: isEn ? 'Printed Image URL' : 'رابط صورة الطباعة',
                    value: finalImageAttr,
                  },
                ]
              : []),
            ...(previewImageAttr
              ? [
                  {
                    key: isEn
                      ? 'Cake Preview Image'
                      : 'صورة شكل الكيكة ثلاثية الأبعاد',
                    value: previewImageAttr,
                  },
                ]
              : []),
          ],
    ...
    return Response.json(
      {
        error: isEn
          ? 'Failed to create order. Please try again or contact us.'
          : 'فشل في إنشاء الطلب. يرجى المحاولة مرة أخرى أو التواصل معنا.',
      },
      {status: 500},
    );
  } catch (globalError: any) {
    console.error(
      '[Custom Cake] UNCAUGHT:',
