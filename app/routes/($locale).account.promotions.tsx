...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  if (intent === 'create_voucher') {
    ...
    if (!code || code.trim().length < 3) {
      return data({ error: isEn ? 'Voucher Code must be at least 3 characters.' : 'رمز القسيمة يجب أن يكون على الأقل ٣ أحرف.' }, { status: 400 });
    }
    if (isNaN(value) || value <= 0) {
      return data({ error: isEn ? 'Discount value must be greater than 0.' : 'قيمة الخصم يجب أن تكون أكبر من 0.' }, { status: 400 });
    }
    if (valueType === 'PERCENTAGE' && value > 100) {
      return data({ error: isEn ? 'Percentage discount value cannot exceed 100%.' : 'قيمة خصم النسبة المئوية لا يمكن أن تتجاوز ١٠٠٪.' }, { status: 400 });
    }
    if (isNaN(minSubtotal) || minSubtotal < 0) {
      return data({ error: isEn ? 'Minimum Subtotal must be a positive number.' : 'الحد الأدنى للطلب يجب أن يكون رقماً إيجابياً.' }, { status: 400 });
    }
    if (formData.get('usageLimit') && (isNaN(usageLimit) || usageLimit <= 0)) {
      return data({ error: isEn ? 'Usage limit must be a positive number.' : 'حد الاستخدام يجب أن يكون رقماً إيجابياً.' }, { status: 400 });
    }
    if (endsAt) {
      ...
      if (expiryDate < today) {
        return data({ error: isEn ? 'Expiry Date cannot be in the past.' : 'تاريخ الانتهاء لا يمكن أن يكون في الماضي.' }, { status: 400 });
      }
    }
    ...
    if (customerEmail || customerTag) {
      if (customerEmail) {
        const cRes = await fetch(`https://${shopDomain}/admin/api/2024-01/graphql.json`, {
          method: 'POST',
        ...
        if (targetedCustomerIds.length === 0) {
          return data({ error: isEn ? 'No customer found matching the specified email.' : 'لم يتم العثور على أي عميل يطابق البريد الإلكتروني المحدد.' }, { status: 400 });
        }
      }
      if (customerTag) {
        const tagRes = await fetch(`https://${shopDomain}/admin/api/2024-01/graphql.json`, {
          method: 'POST',
          headers: { 'X-Shopify-Access-Token': adminToken, 'Content-Type': 'application/json' },
          body: JSON.stringify({
    ...
    const priceRuleInput: any = {
      ...
      value_type: valueType === 'FREE' ? 'percentage' : (valueType === 'PERCENTAGE' ? 'percentage' : 'fixed_amount'),
      value: valueType === 'FREE' ? "-100.0" : `-${value}`,
      customer_selection: targetedCustomerIds.length > 0 ? "prerequisite" : "all",
      starts_at: new Date().toISOString(),
      usage_limit: usageLimit > 0 ? usageLimit : null,
      prerequisite_subtotal_range: minSubtotal > 0 ? { greater_than_or_equal_to: minSubtotal.toString() } : null
    };
    ...
    if (targetedCustomerIds.length > 0) {
        priceRuleInput.prerequisite_customer_ids = targetedCustomerIds.map((id: string) => id.split('/').pop());
    }
    ...
    try {
      ...
      if (!priceRuleRESTId) {
        ...
        else if (typeof prData.errors === 'object') errStr = JSON.stringify(prData.errors);
      ...
      if (branchId !== 'all' || customerTag || (orderType && orderType !== 'ALL')) {
        ...
        if (existingRulesRaw) {
          try { discountRules = JSON.parse(existingRulesRaw); } catch (e) {}
        }
      ...
      await fetch(`https://${shopDomain}/admin/api/2024-01/price_rules/${priceRuleRESTId}/discount_codes.json`, {
        method: 'POST',
        headers: { 'X-Shopify-Access-Token': adminToken, 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
export default function PromotionsDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'vouchers' | 'analytics'>('vouchers');
  ...
  const uniqueBranches = Array.from(new Set((redemptions || []).map((r: any) => r.branch))).filter(Boolean);
  ...
  const totalSalesGenerated = filteredRedemptions.reduce((sum: number, r: any) => sum + r.orderTotal, 0);
  const totalDiscountsGiven = filteredRedemptions.reduce((sum: number, r: any) => sum + r.discountAmount, 0);
  const averageOrderValue = filteredRedemptions.length > 0 ? (totalSalesGenerated / filteredRedemptions.length) : 0;
  const averageDiscountAmount = filteredRedemptions.length > 0 ? (totalDiscountsGiven / filteredRedemptions.length) : 0;

  // Group by code for Top Vouchers
  const voucherStats: Record<string, { code: string; redemptions: number; totalDiscount: number; totalSales: number }> = {};
  filteredRedemptions.forEach((r: any) => {
    if (!voucherStats[r.code]) {
      voucherStats[r.code] = { code: r.code, redemptions: 0, totalDiscount: 0, totalSales: 0 };
    }
  ...
  const topVouchers = Object.values(voucherStats).sort((a, b) => b.redemptions - a.redemptions);
  ...
  const dailyCounts = dateList.map(dateStr => {
    const count = filteredRedemptions.filter((r: any) => r.date.split('T')[0] === dateStr).length;
  ...
  const handleExportCSV = () => {
    const headers = isEn 
      ? ['Order ID', 'Date', 'Voucher Code', 'Customer Name', 'Customer Email', 'Branch', 'Order Total (SAR)', 'Discount Amount (SAR)']
      : ['رقم الطلب', 'التاريخ', 'كود الخصم', 'اسم العميل', 'البريد الإلكتروني', 'الفرع', 'إجمالي الطلب (ر.س)', 'قيمة الخصم (ر.س)'];
    ...
    link.setAttribute("download", `coupon_analytics_${startDate}_to_${endDate}.csv`);
  ...
  return (
    <div className="promotions-dashboard-content" dir={isEn ? 'ltr' : 'rtl'}>
      <div className="flex justify-between items-end mb-10">
        <div>
          ...
          <p className="text-gray-500 font-medium">
            {isEn ? 'Create and track performance of vouchers and branch-specific campaigns' : 'إنشاء وتتبع أداء القسائم وعروض الفروع المخصصة'}
          </p>
        </div>
        {activeTab === 'vouchers' && (
      ...
      <div className="flex border-b border-gray-200 mb-8">
        <button
          onClick={() => { setActiveTab('vouchers'); setSelectedVoucherCode(null); }}
              ...
              <Form method="POST" className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="luxury-field">
                    ...
                    <input 
                      ...
                      title={isEn ? "Alphanumeric, minimum 3 characters" : "أحرف وأرقام فقط، ٣ رموز كحد أدنى"} 
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="luxury-field">
                       <label className="luxury-label">{isEn ? 'Customer Tag / Segment' : 'وسم / شريحة العملاء'}</label>
                       <input name="customerTag" placeholder="e.g. VIP, loyal" className="luxury-input-field" />
                    </div>
                  </div>
                </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center gap-4 flex-wrap">
              <div className="flex flex-col gap-1">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{isEn ? 'Start Date' : 'تاريخ البدء'}</span>
                <input
                  type="date"
                  value={startDate}
              ...
              <div className="flex flex-col gap-1">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{isEn ? 'End Date' : 'تاريخ الانتهاء'}</span>
                <input
                  type="date"
                  value={endDate}
              ...
              <div className="flex flex-col gap-1">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{isEn ? 'Branch' : 'الفرع'}</span>
                <select
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
              ...
              <span className="text-3xl font-black text-[#234745] flex items-baseline gap-1">
                <span className="text-sm font-medium">{isEn ? 'SAR' : 'ر.س'}</span>
                {totalSalesGenerated.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </span>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
              ...
              <span className="text-3xl font-black text-[#234745] flex items-baseline gap-1">
                <span className="text-sm font-medium">{isEn ? 'SAR' : 'ر.س'}</span>
                {totalDiscountsGiven.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </span>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm lg:col-span-2 flex flex-col justify-between">
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-400 font-medium">
                  {isEn ? 'Redemptions per day over time' : 'عدد مرات الاستخدام اليومية'}
                </p>
              </div>
              ...
              <div className="w-full relative h-[180px]">
                {dailyCounts.length > 0 ? (
                  <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="w-full h-full overflow-visible">
                    <defs>
                      <linearGradient id="chartGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#234745" stopOpacity="0.25" />
                        <stop offset="100%" stopColor="#234745" stopOpacity="0.00" />
                      </linearGradient>
                    </defs>
                    {[0, 0.25, 0.5, 0.75, 1].map((r, i) => {
                      ...
                      return (
                        <g key={i} className="opacity-30">
                          <line x1={paddingLeft} y1={y} x2={chartWidth - paddingRight} y2={y} stroke="#ccc" strokeDasharray="3 3" />
                          <text x={isEn ? paddingLeft - 8 : chartWidth - paddingRight + 8} y={y + 4} textAnchor={isEn ? "end" : "start"} className="text-[9px] fill-gray-500 font-bold font-en">{labelVal}</text>
                        </g>
                      );
                    })}
                    
                    {dailyCounts.length > 1 && [0, 0.5, 1].map((ratio, i) => {
                      const index = Math.round(ratio * (dailyCounts.length - 1));
                      const dPoint = points[index];
                      if (!dPoint) return null;
                      return (
                        <text key={i} x={dPoint.x} y={chartHeight - 10} textAnchor="middle" className="text-[9px] fill-gray-400 font-bold font-en">
                          {new Date(dPoint.date).toLocaleDateString(isEn ? 'en-US' : 'en-US', { month: 'short', day: 'numeric' })}
                        </text>
                      );
                    })}
                    ...
                    {points.length > 1 && (
                      <>
                        ...
                        <path d={pathData} fill="none" stroke="#234745" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
                      </>
                    )}

                    {points.map((p, i) => (
                      <g key={i} className="group cursor-pointer">
                        <circle cx={p.x} cy={p.y} r="4" fill="#234745" stroke="#fff" strokeWidth="2" />
                  ...
                  <div className="w-full h-full flex items-center justify-center text-sm font-medium text-gray-400">
                    {isEn ? 'No data for selected period' : 'لا توجد بيانات للفترة المحددة'}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-4">
                  {Object.entries(
                    filteredRedemptions.reduce((acc: Record<string, number>, curr: any) => {
            ...
            {selectedVoucherCode ? (
              <div className="space-y-6">
                ...
                <table className="w-full text-start">
                  <thead>
                    <tr className="bg-gray-50 text-[11px] uppercase tracking-wider text-gray-400 border-b">
                      <th className="px-6 py-4 font-bold">{isEn ? 'Order' : 'الطلب'}</th>
                      <th className="px-6 py-4 font-bold">{isEn ? 'Date' : 'التاريخ'}</th>
                      <th className="px-6 py-4 font-bold">{isEn ? 'Customer' : 'العميل'}</th>
                      <th className="px-6 py-4 font-bold">{isEn ? 'Branch' : 'الفرع'}</th>
                      <th className="px-6 py-4 font-bold">{isEn ? 'Total' : 'المجموع'}</th>
                      <th className="px-6 py-4 font-bold">{isEn ? 'Discount' : 'الخصم'}</th>
                    </tr>
                  </thead>
                      ...
                      .map((red: any, idx) => (
                        <tr key={idx} className="hover:bg-gray-50/50">
                          ...
                          <td className="px-6 py-4 text-gray-400 font-en">
                            {new Date(red.date).toLocaleDateString(isEn ? 'en-US' : 'ar-SA', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                          </td>
                          <td className="px-6 py-4">
                            <span className="block text-[#1a1a1a]">{red.customerName}</span>
                            <span className="block text-[11px] text-gray-400 font-en">{red.customerEmail}</span>
                          </td>
                          <td className="px-6 py-4 text-gray-500">{red.branch}</td>
                          <td className="px-6 py-4 font-black font-en">{red.orderTotal.toFixed(2)} SAR</td>
                          <td className="px-6 py-4 text-emerald-600 font-black font-en">-{red.discountAmount.toFixed(2)} SAR</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-black text-[#234745]">
                    {isEn ? 'Top Performing Vouchers' : 'أكواد الخصم الأعلى أداءً'}
                  </h3>
                  <p className="text-xs text-gray-400 font-medium">
                    {isEn ? 'Click any voucher row to drill down into redemption history' : 'انقر على أي صف لمشاهدة تفاصيل عمليات الاستخدام'}
                  </p>
                </div>
                ...
                <table className="w-full text-start">
                  <thead>
                    <tr className="bg-gray-50 text-[11px] uppercase tracking-wider text-gray-400 border-b">
                      <th className="px-6 py-4 font-bold">{isEn ? 'Code' : 'الرمز'}</th>
                      <th className="px-6 py-4 font-bold">{isEn ? 'Total Redemptions' : 'إجمالي الاستخدام'}</th>
                      <th className="px-6 py-4 font-bold">{isEn ? 'Sales Generated' : 'المبيعات المحققة'}</th>
                      <th className="px-6 py-4 font-bold">{isEn ? 'Discounts Applied' : 'إجمالي الخصومات'}</th>
                      <th className="px-6 py-4 font-bold">{isEn ? 'Action' : 'الإجراء'}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y font-medium text-sm">
                    {topVouchers.map((v: any) => (
                      ...
                      >
                        <td className="px-6 py-4 font-bold text-[#234745] font-en">{v.code}</td>
                        <td className="px-6 py-4 font-black font-en">{v.redemptions}</td>
                        <td className="px-6 py-4 font-black font-en">{v.totalSales.toFixed(2)} SAR</td>
                        <td className="px-6 py-4 text-emerald-600 font-black font-en">{v.totalDiscount.toFixed(2)} SAR</td>
                        <td className="px-6 py-4">
                    ...
                    {topVouchers.length === 0 && (
                      <tr>
                        <td colSpan={5} className="px-6 py-8 text-center text-gray-400 font-medium">
                          {isEn ? 'No voucher redemptions found for this date range.' : 'لا توجد عمليات استخدام في هذا النطاق الزمني.'}
                        </td>
                      </tr>
                    )}
                  </tbody>
