import { data, type LoaderFunctionArgs, useLoaderData, Link, useRouteLoaderData, useSearchParams } from 'react-router';
...
export const meta = ({ matches }: { matches: any }) => {
  return [{ title: getShopTitle('Export & Wholesale | Saadeddin Pastry', matches) }];
};
...
export async function loader({ context, request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    if (!products) {
      return data({ products: null, collections: null, error: "GraphQL query returned null." });
    }
...
export default function ExportPage() {
  ...
  return (
    <div className="bg-[#FEF8EB] min-h-screen" dir={isEn ? 'ltr' : 'rtl'}>
        ...
        <div className={`relative z-10 max-w-[1440px] w-full mx-auto px-6 sm:px-10 lg:px-16 py-12 flex flex-col items-start ${isEn ? 'text-left' : 'text-right'}`}>
          <div className={`max-w-[720px] flex flex-col items-start ${isEn ? 'text-left' : 'text-right'}`}>
            ...
            <div className="inline-flex items-center gap-2 px-6 py-1.5 rounded-full bg-[#C5A96A] text-[#1A3533] font-bold text-[13px] sm:text-[14px] mb-6 shadow-md">
              ...
              <span>{isEn ? 'Saudi Heritage Since 1960' : 'تراث سعودي من ١٩٦٠'}</span>
              <span className="text-[#1A3533]/60">—</span>
            </div>
            ...
            >
              {isEn ? (
                <>Saudi Taste,<br />Reaching Around the World</>
              ) : (
                <>طعم سعودي،<br />يصل حول العالم</>
              )}
            </h1>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
              <a
                ...
                style={{ fontFamily: "'GE Dinar One', sans-serif", fontWeight: 700 }}
              >
      ...
      <section id="export-catalog" className="py-14 max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12 text-right">
        {/* Section Header */}
        <div className="text-center mb-10">
          ...
          <p
            ...
            style={{ fontFamily: "'GE Dinar One', sans-serif" }}
          >
            {isEn ? 'Selected export range with special international specs' : 'منتجات مختارة للتصدير - بمواصفات خاصة'}
          </p>
        </div>
        ...
        {!products ? (
          <div className="py-16 text-center font-bold text-[#234745]">
            {isEn ? 'Loading export catalog...' : 'جاري تحميل منتجات التصدير...'}
          </div>
        ) : (
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div className="flex-1 min-w-0 w-full lg:order-2">
              ...
              <div className="lg:hidden flex flex-col gap-4 mb-4" dir={isEn ? 'ltr' : 'rtl'}>
                <div className="flex items-center justify-between w-full gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    <span className="text-[#7D7D7D] text-[13px] font-medium whitespace-nowrap" style={{ fontFamily: "'GE Dinar One', sans-serif" }}>
                    ...
                    <div className="flex items-center bg-white border border-[#BBCFCD]/60 rounded-[8px] px-3 py-1.5 relative w-[135px]">
                      ...
                      >
                        <option value="RELEVANCE|false">{isEn ? 'Featured' : 'الأكثر مبيعا'}</option>
              ...
              <div className={`hidden lg:flex ${isEn ? 'flex-row' : 'flex-row-reverse'} items-center justify-between gap-4 mb-6 w-full`} dir={isEn ? 'ltr' : 'rtl'}>
                <div className="flex items-center gap-2.5 shrink-0">
                  <span className="text-[#7D7D7D] text-[14px] font-bold" style={{ fontFamily: "'GE Dinar One', sans-serif" }}>
                    {isEn ? 'Sort by:' : 'ترتيب حسب:'}
                  </span>
                    ...
                    {nodes.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {nodes.map((product: any, index: number) => (
                          <ProductItem key={product.id || index} product={product} loading={index < 6 ? 'eager' : 'lazy'} isExport={true} />
                        ))}
                      </div>
                    )}
      ...
      <section className="w-full bg-white py-16 border-t border-gray-100">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <span className="text-[#906B51] text-[18px] font-medium tracking-wide uppercase mb-1 block" style={{ fontFamily: "'GE Dinar One', sans-serif" }}>
              {isEn ? 'Quality & Global Logistics Assurance' : 'ضمانات الجودة والشحن الدولي'}
            </span>
            <h2
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-[#ffffff] rounded-[12px] py-[24px] px-[12px] border border-[#234745] text-center flex flex-col items-center hover:shadow-md transition-all">
              <h3 className="text-[26px] font-bold text-[#234745] mb-2" style={{ fontFamily: "'Bahij Janna', serif" }}>
                {isEn ? 'Global Cold Shipping' : 'شحن مبرد عالمي'}
              </h3>
              <p className="text-[#9FB7AE] text-[14px] font-normal leading-relaxed" style={{ fontFamily: "'GE Dinar One', sans-serif" }}>
                {isEn
          ...
          <div className="lg:col-span-6 bg-white text-[#234745] rounded-[12px] p-6 sm:p-8 shadow-lg">
            ...
            {formSubmitted ? (
              <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 p-6 rounded-2xl text-center">
                ...
                <h4 className="font-bold text-[18px] mb-1">{isEn ? 'Request Received!' : 'تم استلام طلبك بنجاح!'}</h4>
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                ...
                <p className="text-[12px] text-[#9FB7AE] text-center pt-1">
                  {isEn ? 'We will respond within 24 business hours' : 'سنتواصل معك في أسرع وقت خلال 24 ساعة'}
                </p>
              </form>
            )}
          </div>
