...
export async function action({request, context}: ActionFunctionArgs) {
  ...
  try {
    ...
    const rawShop =
      context.env.SHOPIFY_ADMIN_DOMAIN ||
    ...
    try {
      ...
      const receiverEmail =
        context.env.CONTACT_RECEIVER_EMAIL ||
    ...
    try {
      ...
      shopifyFormBody.append(
        'contact[Body]',
        `Name: ${fullName}\nPhone: ${mobile}\nEmail: ${email}\nSubject: ${subject}\nOrder Number: ${orderNumber || 'N/A'}\n\nMessage:\n${message}`,
      );

      const shopifyResponse = await fetch(
        `https://${shopDomain}/contact#contact_form`,
        {
          method: 'POST',
          headers: {
      ...
      console.log(
        '[SHOPIFY NATIVE CONTACT FORM SUBMIT STATUS]',
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (!document.getElementById(scriptId)) {
        ...
        script.id = scriptId;
        script.src =
          'https://cdn.shopify.com/shopifycloud/shopify_forms/latest/shopify_forms.js';
        script.async = true;
    ...
    >
      ...
      <div className="relative w-full bg-[#234745] overflow-hidden flex flex-col items-center justify-center text-white px-[80px] py-10">
        ...
        <div className="relative z-10 text-center">
          <p
            className="text-[16px] font-medium text-[#9FB7AE]"
            style={{
          ...
          <p
            className="text-[14px] md:text-[16px] font-medium text-[#9FB7AE] mt-2"
            style={{
              fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
              lineHeight: '100%',
            }}
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-6 mt-10 md:mt-16 relative z-20 pb-20">
        ...
        <div className="flex flex-col md:flex-row gap-10 lg:gap-20 items-start">
          ...
          <div className="w-full md:w-[55%] space-y-12">
            <section>
              <h2
                className="text-[28px] md:text-[38px] font-bold text-[#1F413F] mb-10"
                style={{
                  fontFamily: "'EnglishDigits', 'Bahij Janna', sans-serif",
                  lineHeight: '100%',
                }}
              ...
              <div className="space-y-0">
                <InfoRow
                  label={isEn ? 'WhatsApp' : 'واتساب'}
                  value="+966 9200 17070"
                  isEn={isEn}
                  isLtrValue
                />
                <InfoRow
            ...
            <section>
              ...
              <div className="space-y-0">
                <InfoRow
                  label={isEn ? 'Everyday' : 'يومياً'}
                  value={isEn ? '7:30 AM - 4:00 PM' : '7:30 ص - 4:00 م'}
                  isEn={isEn}
                  forceSans
                />
              </div>
          ...
          <div
            id="contact-form"
            className="w-full md:w-[45%] bg-white border border-[#234745] shadow-sm overflow-hidden scroll-mt-10"
          ...
          >
            ...
            >
              {actionData?.success === true ? (
                <div
                  className="p-10 w-full text-center flex flex-col items-center justify-center gap-6"
                ...
                >
                  ...
                  <h2
                    className="text-[28px] font-bold text-[#234745]"
                    style={{
                      fontFamily: "'EnglishDigits', 'Bahij Janna', sans-serif",
                    }}
                  >
                  ...
                  <p
                    className="text-[16px] text-gray-500 max-w-[400px] leading-relaxed"
                    style={{
                      fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
                      fontWeight: 500,
                    }}
                ...
                <>
                  <h2
                    className="text-[20px] md:text-[26px] font-bold text-[#234745]"
                    style={{
                      fontFamily: "'EnglishDigits', 'Bahij Janna', sans-serif",
                      lineHeight: '100%',
                      marginBottom: '25px',
                  ...
                  {actionData?.success === false && (
                    <div
                      className="mb-6 p-4 rounded-xl bg-red-50 text-red-800 border border-red-200 text-center font-bold text-[15px]"
                  ...
                  >
                    ...
                    >
                      ...
                      <FormField
                        name="mobile"
                        label={isEn ? 'Mobile Number' : 'رقم الجوال'}
                        placeholder={isEn ? '05X XXX XXXX' : 'رقم الجوال'}
                        required
                        isEn={isEn}
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                      <label
                        className="text-[15px] font-medium text-[#1F413F] flex items-center gap-1"
                        style={{
                          fontFamily:
                      ...
                      <div className="relative">
                        <select
                          name="subject"
                          required
                          className={`w-full h-[58px] px-4 md:px-6 rounded-xl border border-gray-200 focus:border-[#1F413F] outline-none bg-white text-[14px] md:text-[16px] appearance-none transition-all ${isEn ? 'text-left' : 'text-right'}`}
                          dir={isEn ? 'ltr' : 'rtl'}
                          style={{
                            fontFamily:
                              "'EnglishDigits', 'GE Dinar One', sans-serif",
                        ...
                        >
                          <option value="">
                            {isEn ? 'Select Subject' : 'إختر الموضوع'}
                          </option>
                          <option value="Inquiry">
                            {isEn ? 'Inquiry' : 'استفسار'}
                          </option>
                          <option value="Sales">
                            {isEn ? 'Sales' : 'مبيعات'}
                          </option>
                          <option value="Recruitment">
                            {isEn ? 'Recruitment' : 'توظيف'}
                          </option>
                          <option value="Complaint">
                            {isEn ? 'Complaint' : 'شكوى'}
                          </option>
                          <option value="Special Order">
                            {isEn ? 'Special Order' : 'طلبية خاصة'}
                          </option>
                          <option value="Catering">
                            {isEn ? 'Catering' : 'كاتيرنج'}
                          </option>
                          <option value="Service Proposal">
                            {isEn ? 'Service Proposal' : 'عرض خدمة'}
                          </option>
                          <option value="Other">
                            {isEn ? 'Other' : 'اخرى'}
                          </option>
                        ...
                        <div
                          className={`absolute top-1/2 -translate-y-1/2 pointer-events-none ${isEn ? 'right-4' : 'left-4'}`}
                        >
                          <svg
                            width="14"
                            height="14"
                    ...
                    <FormField
                      name="orderNumber"
                      label={
                        isEn ? 'Order Number (Optional)' : 'رقم الطلب (إختياري)'
                      }
                      placeholder="sd-asdqo142"
                      isEn={isEn}
                      forceLtr
                    />
...
function FormField({
  ...
  return (
    <div
      className={`flex flex-col gap-3 ${isEn ? 'text-left' : 'text-right'}`}
      style={{width: '100%'}}
    >
      <label
        className="text-[14px] font-medium text-[#1F413F] flex items-center gap-1"
        style={{
          fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
          lineHeight: '100%',
        }}
      ...
      <input
        ...
        style={{
          fontFamily: forceLtr ? 'sans-serif' : "'GE Dinar One', sans-serif",
          fontWeight: 500,
        }}
      />
    </div>
...
function InfoRow({
  ...
  return (
    <div className="flex items-center justify-between py-5 border-b border-[#1F413F]/5">
      <span
        className="text-[14px] md:text-[16px] font-medium text-[#9FB7AE]"
        style={{
          fontFamily: "'EnglishDigits', 'GE Dinar One', sans-serif",
          lineHeight: '100%',
        }}
