...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (stage) {
      console.log(`[Notification Webhook] Triggering ${stage} for Order #${payload.order_number}`);
...
export async function loader() {
  return Response.json({ status: 'Notification Webhook active. Use POST.' }, { status: 200 });
}
