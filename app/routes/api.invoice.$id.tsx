...
export async function loader({ params, context }: LoaderFunctionArgs) {
    ...
    return json({
        order: {
            id: orderNode.name,
            date: new Date(orderNode.processedAt).toLocaleDateString(isEn ? 'en-US' : 'ar-SA-u-nu-latn', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: '2-digit',
            ...
            subtotal: parseFloat(orderNode.subtotalPriceSet?.shopMoney?.amount || '0'),
            tax: parseFloat(orderNode.totalTaxSet?.shopMoney?.amount || '0'),
            shipping: parseFloat(orderNode.totalShippingPriceSet?.shopMoney?.amount || '0'),
            ...
            items: orderNode.lineItems.edges.map(({ node: item }: any) => ({
                ...
                total: parseFloat(item.originalUnitPriceSet?.shopMoney?.amount || '0') * item.quantity
            }))
        },
        locale,
        isEn
...
export default function InvoicePage() {
    ...
    if (!order.isPaid) {
        return (
            <div className="min-h-screen bg-[#FEF8EB] flex items-center justify-center p-6" dir={isEn ? 'ltr' : 'rtl'}>
    ...
    return (
        <div className="bg-gray-100 min-h-screen py-8 px-4" dir={isEn ? 'ltr' : 'rtl'}>
            ...
            <div className="max-w-[800px] mx-auto bg-white rounded-t-2xl border-b border-gray-100 px-6 py-4 flex justify-between items-center print:hidden">
                <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-[14px] font-bold text-[#234745] hover:underline">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={isEn ? 'rotate-180' : ''}>
            ...
            {/* Print Area */}
            <div className="max-w-[800px] mx-auto bg-white p-8 lg:p-12 shadow-sm rounded-b-2xl print:shadow-none print:rounded-none print:p-0 text-[#1a1a1a]" id="invoice-sheet">
                ...
                <div className="flex justify-between items-start border-b-2 border-gray-200 pb-6 mb-8">
                    ...
                    <div className="text-right">
                        <span className="text-[20px] font-black text-[#d4a06a]">{isEn ? 'SAADEDDIN' : 'سعد الدين'}</span>
                        <span className="text-[11px] text-gray-400 block font-bold mt-1">VAT Reg No: {vatNumber}</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8 mb-8 text-[13px]">
                    <div>
                        ...
                        <div className="flex flex-col gap-1.5">
                            <div><span className="font-bold">{isEn ? 'Invoice No: ' : 'رقم الفاتورة: '}</span>{order.id}</div>
                            <div><span className="font-bold">{isEn ? 'Issue Date: ' : 'تاريخ الإصدار: '}</span>{order.date}</div>
                            <div><span className="font-bold">{isEn ? 'Payment: ' : 'طريقة الدفع: '}</span>{order.paymentMethod}</div>
                            <div>
                                <span className="font-bold">{isEn ? 'Status: ' : 'الحالة: '}</span>
                    ...
                    <div>
                        ...
                        <div className="flex flex-col gap-1.5">
                            <div><span className="font-bold">{isEn ? 'Name: ' : 'الاسم: '}</span>{order.customerName}</div>
                            {order.customerPhone && <div><span className="font-bold">{isEn ? 'Phone: ' : 'الجوال: '}</span>{order.customerPhone}</div>}
                            {order.customerCity && <div><span className="font-bold">{isEn ? 'City: ' : 'المدينة: '}</span>{order.customerCity}</div>}
                            {order.customerAddress && <div><span className="font-bold">{isEn ? 'Address: ' : 'العنوان: '}</span>{order.customerAddress}</div>}
                        </div>
                    </div>
                </div>
                ...
                <table className="w-full text-[13px] border-collapse mb-8">
                    <thead>
                        <tr className="border-b-2 border-[#234745] text-gray-400 font-bold">
                            <th className="py-3 text-start">{isEn ? 'Description' : 'الوصف'}</th>
                            <th className="py-3 text-center w-[80px]">{isEn ? 'Qty' : 'الكمية'}</th>
                            <th className="py-3 text-end w-[120px]">{isEn ? 'Unit Price' : 'سعر الوحدة'}</th>
                            <th className="py-3 text-end w-[120px]">{isEn ? 'VAT (15%)' : 'الضريبة'}</th>
                            <th className="py-3 text-end w-[120px]">{isEn ? 'Total' : 'الإجمالي'}</th>
                        </tr>
                    </thead>
                        ...
                        {order.items.map((item: any, idx: number) => {
                            ...
                            return (
                                <tr key={idx} className="align-middle">
                                    <td className="py-3 font-bold text-[#1a1a1a]">{item.title}</td>
                                    <td className="py-3 text-center">{item.quantity}</td>
                                    <td className="py-3 text-end font-mono" dir="ltr">{itemPriceExclVat.toFixed(2)} {order.currency}</td>
                                    <td className="py-3 text-end font-mono text-gray-500" dir="ltr">{(itemVat * item.quantity).toFixed(2)} {order.currency}</td>
                                    <td className="py-3 text-end font-bold font-mono" dir="ltr">{item.total.toFixed(2)} {order.currency}</td>
                                </tr>
                            );
                        })}
                    </tbody>
                ...
                <div className="flex justify-end border-t border-gray-100 pt-6">
                    ...
                    <div className="w-full md:w-[320px] flex flex-col gap-2.5 text-[13px] font-medium">
                        <div className="flex justify-between items-center text-gray-500">
                            ...
                            <span className="font-mono" dir="ltr">{(order.subtotal / 1.15).toFixed(2)} {order.currency}</span>
                        </div>
                        
                        {order.shipping > 0 && (
                            <div className="flex justify-between items-center text-gray-500">
                                <span>{isEn ? 'Delivery Fee' : 'رسوم التوصيل'}</span>
                                <span className="font-mono" dir="ltr">{(order.shipping).toFixed(2)} {order.currency}</span>
                            </div>
                        )}

                        <div className="flex justify-between items-center text-gray-500">
                            <span>{isEn ? 'Total VAT (15%)' : 'ضريبة القيمة المضافة (15%)'}</span>
                            <span className="font-mono" dir="ltr">{order.tax.toFixed(2)} {order.currency}</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center text-[18px] font-black text-[#234745] pt-1">
                            ...
                            <span className="font-mono" dir="ltr">{order.total.toFixed(2)} {order.currency}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="border-t border-gray-100 pt-8 mt-12 text-center text-[11px] text-gray-400 leading-relaxed font-bold">
                    <div>{isEn ? 'Thank you for shopping with Saadeddin!' : 'نشكركم لتسوقكم من حلويات سعد الدين!'}</div>
                    <div className="mt-1" dir="ltr">www.saadeddin.com</div>
                </div>
